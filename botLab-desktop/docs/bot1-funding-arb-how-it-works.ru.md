...
- Бот встаёт на недостающую сторону на GMX V2 (какую именно, каждый раз говорит знак ставок) и
  получает плату. Чтобы не зависеть от цены, он
  одновременно встаёт на противоположную сторону того же актива на Hyperliquid. Две позиции
  называются **ногами**; вместе они нейтральны к цене.
...
- Базы бот копит сам, час за часом, и **первые четыре недели не входит**: пока не наблюдено
  684 часа из 720, считать нечего. Затем раз в сутки он выбирает рынок и размер правилом входа,
  держит позицию, а раз в сутки спрашивает правилом выхода: держать, в кэш или переложиться.
...
flowchart TD
    ...
    B --> C["Накопление: каждые 5 минут тик пишет снимок<br/>и наблюдает базу фандинга часа по каждому рынку"]
    C --> D{{"Ворота: на окне 720 ч база наблюдена<br/>хотя бы в 684 ч (95%)?"}}
...
3. **Накопление.** Каждые 5 минут бот читает обе биржи, пишет снимок в архив и записывает базу
   фандинга текущего часа по каждому рынку. Базы копятся только вперёд.
4. **Ворота.** На окне в 720 часов база должна быть наблюдена хотя бы в 684 часах. Пока нет,
   рынок в решение не входит, и первые четыре недели наблюдения сделок не будет.
...
7. **Удержание и выход.** Каждый тик работает сторож залога, раз в сутки правило выхода: держать,
...
sequenceDiagram
    ...
    opt автомат взведён
        ...
        E->>D: строка снимка в архив; строка пропуска, если перерыв
    end
    E->>E: непрерывность, ворота снабжения, сторож залога
    alt запас до ликвидации тонкий
        E->>D: закрытие сделки без ожидания каданса; строка паспорта
    else каданс подошёл (раз в 24 ч)
        E->>E: правило входа или правило выхода
        E->>D: строка решения; при сделке строка паспорта
    end
...
- **Вселенная фиксирована.** Пять рынков: двуногие ETH и BTC (GMX V2 на Arbitrum против
  ...
  короткая на GMX без противовеса.
...
- **Базы наблюдаются всегда.** Журнал баз пишется на каждом опросе, даже при выключенном автомате:
  окно в 720 часов иначе не набралось бы никогда. Внутри часа опрос проходит двенадцать раз, и
  побеждает первое наблюдение: журнал дописывается, а не переписывается.
...
Взять базы из истории нельзя: индексатор за 71 день сдвинул 40.4% часов при переиндексации, и
бэкфилл запрещён решением владельца 31.08.2026. Единственный путь: наблюдать вперёд, час за часом,
из живого `markets/info`.

## Ворота: 684 часа из 720

Каждый тик автомат берёт последние 720 часов кадра по каждому рынку и спрашивает: в скольких из
них наблюдена база нашей стороны? Порог 95%, то есть 684 часа. Рынок ниже порога ворота не
...
Сроки. Час наблюдения прибавляет ровно один покрытый час, быстрее набрать нельзя: от начала
наблюдения до первого возможного входа около 28.5 суток. Любой перерыв опроса (сон машины,
выключенное приложение) сдвигает дату вправо. **Первые четыре недели после того, как приложение
начало наблюдать базы, сделки не будет, и это норма, а не поломка.** Наблюдение идёт и при
выключенном автомате, поэтому если приложение работало до взвода, часть окна уже набрана.

Что при этом видно. На Обзоре карточка бота показывает жетон и строку «ИЩЕТ ВХОД · базы наблюдены
N из 684 ч». На пульте автомата: причина словами («наблюдённых баз фандинга на окне не хватает») и
числами («рынок: база наблюдена в N из 720 ч окна, требуется 95%»), колонка «Ворота снабжения»
(рынков в обходе, прошло ворота, лучшее покрытие при требуемых 95%, горизонт окна 720 ч) и
плашка с датой: сколько часов не хватает и раньше какого момента порог не наберётся.
...
%%{init: {"themeVariables": {"fontSize": "12px"}, "flowchart": {"nodeSpacing": 26, "rankSpacing": 18, "diagramPadding": 4, "wrappingWidth": 380}}}%%
flowchart TD
    A["Вселенная: 5 рынков"] --> B{{"Ворота снабжения: 720 строк истории,<br/>база наблюдена в 684 ч?"}}
...
- **Не берёт базы из истории.** Бэкфилл запрещён; журнал баз пишется только живым опросом.
...
- **Не решает чаще раза в сутки.** Кроме сторожа залога: его отказ невосстановим следующим
...
У одноногой схемы нога одна: короткая на GMX с залогом в самом активе, противовеса нет. Плечо 1
на каждую ногу, ноционал ноги равен размеру сделки.
...
плеча 2 или на удержании, когда цена ушла против ноги. Тонкий запас на открытой сделке (`margin_thin`)
...
- **Честность счёта**, четыре замера, и каждый означает, что показанная прибыль больше настоящей:
  котируемый поток против полученного и доля удержания (замер: при слоте $2000 на рынок остаётся
  около 8.8% котируемого потока, при $10 000 около 6.3%); заявленный и фактический размер рядом;
...
## Правило выхода раз в сутки

На тике решения (раз в 24 часа) правило выхода сравнивает три числа в одних единицах, долларах
за 720 часов вперёд:

- **держать** = брутто текущей позиции на горизонте, по последним 720 часам ставок и баз, при
  зафиксированном размере;
- **в кэш** = ноль;
...
- **Ворота те же, что у входа.** Отказ источника откладывает решение целиком, а не только ветку
  перекладки: без источника «альтернатив нет» выглядело бы как «держим».
...
| Тикет включения | Показывает, что автомат делает без вас, и замороженные параметры: правило входа, капитал $2500, плечо 1, запас до ликвидации 50%, каданс 24 ч, срок годности 72 ч, покрытие баз 95%, «убыток: не ограничен», «опрос заводится на буте: да»; подтверждение одной кнопкой |
...
| Штамп свежести и опрос | Пилюля LIVE мигает на приход снимка, «УСТАРЕЛО» после 15 минут без данных, штамп «данные по состоянию на UTC»; интервал опроса 1, 5 или 15 минут; кнопка обновить сейчас |
| Обзор | Карточка бота с точкой, чипом и строкой состояния («ИЩЕТ ВХОД · базы наблюдены N из 684 ч»), кнопка «Пульт автомата» (ведёт к тикету) или «Остановить автомат» (два шага) |
...
- **Не берёт базы из истории.** Только живое наблюдение вперёд; первые четыре недели наблюдения
  сделок нет.
...
| Одноногая схема | Короткая нога на GMX с залогом в самом активе; противовеса нет |
...
| Окно, горизонт | 720 часов (30 суток): столько прошлых часов берёт правило и на столько часов вперёд оценивает доход |
| Ворота снабжения | Проверки, без которых рынок в решение не входит: 720 строк истории и база, наблюдённая в 684 часах |
| Покрытие баз | Доля часов окна, в которых наблюдена база нашей стороны; порог 95% |
| Тик | Один опрос бирж с начислением, записью и названным исходом; раз в 5 минут по умолчанию |
| Каданс | Как часто зовутся дорогие правила: раз в 24 часа |
...
| `src/engine/fa/bases.js` | Журнал наблюдённых баз, перенос в кадр, покрытие окна; запрет бэкфилла |
...
| `src/engine/assemble.js`, `src/engine/sources.js`, `src/engine/universe.js` | Снимки и кадры, доступ к GMX и Hyperliquid, вселенная пяти рынков |
