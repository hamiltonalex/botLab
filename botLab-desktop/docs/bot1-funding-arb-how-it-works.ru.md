...
- Базу текущего часа бот наблюдает сам на каждом опросе, а часы окна, которых не видел, доливает
  историей индексатора с проверкой тождества сторон: **ворота (684 часа из 720) проходят на первом
  обновлении кадра**, в первые два часа после запуска, а не через четыре недели. Затем раз в сутки
  он выбирает рынок и размер правилом входа, держит позицию, а раз в сутки спрашивает правилом
  выхода: держать, в кэш или переложиться.
...
4. **Ворота.** На окне в 720 часов база должна быть известна хотя бы в 684 часах, живьём или
   доливом. Пока нет, рынок в решение не входит; обычно ворота проходят на первом обновлении
   кадра, в первые два часа после запуска.
...
дырой; каждый час помнит, откуда взята база, и интерфейс не называет долитое наблюдённым. Дрейф
...
Сроки. Долив закрывает часы окна без наблюдения на первом обновлении кадра, то есть не позже двух
часов после запуска, если у индексатора эти часы есть; ожидания в 28.5 суток больше нет. Дыры
...
часа отсчитывается от него; значит первая бумажная сделка возможна в первые два часа после
запуска.** Наблюдение идёт и при выключенном автомате.
...
горизонт окна 720 ч, часов окна долито из индексатора N из 720) и плашка: сколько часов не
