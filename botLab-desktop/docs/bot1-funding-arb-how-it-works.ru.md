...
Всего семь вариантов. Сначала бот выбирает направление A или B для BTC и для ETH по текущему
чистому спреду ставок (`net_A >= net_B` выбирает A). Затем оценивает размер и нетто этих двух
вариантов и трёх одноногих схем на историческом окне и финансирует первый по нетто. Второе
...
В таблице видны семь схем. Два направления, не выбранные по текущим ставкам, помечены отдельно:
полный расчёт размера и нетто для них не выполнялся. Для остальных показаны полученные движком
