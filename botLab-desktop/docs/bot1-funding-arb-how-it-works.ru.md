...
[English version](bot1-funding-arb-how-it-works.en.md) · Документ соответствует коду на 04.09.2026.
...
Перед входом бот просчитывает все заложенные варианты: фандинг обеих ног, стоимость займа на
GMX, комиссии, проскальзывание по стакану, свободную ликвидность площадок и влияние собственного
объёма на ставку. Размер он тоже подбирает сам, от $500 до $2500 на ногу. В итоге открывает только
лучший вариант или не входит вообще, если экономика не сходится: нетто за горизонт обязано окупить
полный круг издержек.

Торговые площадки две: GMX V2 и Hyperliquid. GMX бот проверяет в сетях Arbitrum и Avalanche.
Binance используется только как источник справочной цены, сделки там не открываются.

Состав рынков бот собирает сам, живым отбором, и с версии 0.4.0 это поведение по умолчанию.
Раньше вселенная была зашита пятью строками: BTC и ETH в двух парных схемах плюс три одноногие
схемы на GMX (ETH в Arbitrum, BTC в Arbitrum, ETH в Avalanche). Теперь бот берёт все бессрочные
рынки GMX в Arbitrum и Avalanche и отсеивает те, на которых работать нельзя. На живом снимке
18.09.2026 из 151 рынка осталось 50, и вместе с пятью зашитыми именами в срез правил ушло 52
инструмента.

Почему это важно числом: те же правила и тот же капитал $2500 на двух монетах дают около $40 в год,
а на полусотне имён около $560. Дело не в том, что правило стало лучше, а в том, что ему есть из
чего выбирать: в те часы, когда на BTC и ETH платить некому, платят на других монетах.

Отбор отсеивает рынок по пяти причинам, и каждая видна в интерфейсе своим кодом: рынок не
бессрочный, рынок не листингован, монеты нет на Hyperliquid, свободного места на площадке меньше
требуемого, и наша доля в открытом интересе рынка вышла бы выше порога (по умолчанию 10%). Последнее
ворото самое важное: оно не пускает бота в рынок, который он своим же размером и раздавит. На том же
снимке отказов было 101: доля интереса 55, нет на бирже 24, мало места 12, не бессрочный 8, не
листингован 2.

Зашитые пять имён никуда не делись. Они остаются запасом: если отбор недоступен, список поднимается
из сохранённого файла профиля, а если нет и его, из тех же пяти строк. Новые рынки берутся только
двуногой схемой, три одноногие схемы остаются на прежних именах.

Дальше всё как раньше. Для каждого парного рынка бот выбирает направление A или B по среднему
чистому спреду ставок за окно оценки: у какого направления средняя за 720 часов выше, то и берётся.
Затем оценивает размер и нетто всех вариантов на том же окне и финансирует первый по нетто. Второе
направление каждого парного рынка отдельную оптимизацию размера не проходит.

Направление считается по окну, а не по ставкам текущей минуты, и это существенно. Ставки ходят
каждый час, и выбор по последнему наблюдению переворачивался бы около четырёх раз в сутки, то есть
бот мерил бы размер и доход на окне в месяц для той ноги, которую выбрала одна минута. Средняя по
окну за тот же год переворачивается раз в три недели.

После входа раз в сутки и по событию он сравнивает удержание с теми же альтернативами и с выходом
в кэш, а каждый тик сторожит запас до ликвидации обеих ног и просадку накопленного результата: два
круга издержек, отданные от пика, закрывают сделку. Всё это на бумаге: заявки на биржи не уходят.
...
- На бессрочных рынках одна сторона всегда переполнена, а другой не хватает. Биржа заставляет
  ...
  фандингом.
- Бот встаёт на недостающую сторону на GMX V2 (какую именно, говорит знак ставок за окно оценки) и
...
flowchart TD
    ...
    F --> G["Удержание. Каждый тик: сторож залога, стоп по просадке, снимок в архив.<br/>Раз в сутки: правило выхода"]
    G -- "держать" --> G
    G -- "в кэш, тонкий запас или стоп по просадке" --> C
    G -- "переложиться" --> F
...
7. **Удержание и выход.** Каждый тик работают сторож залога и стоп по просадке, раз в сутки и по
   событию правило выхода: держать, в кэш или переложиться. Тонкий запас до ликвидации закрывает
   сделку, не дожидаясь суток; так же закрывает её стоп, когда накопленный результат сделки отдал
   от своего пика два круга издержек.
...
sequenceDiagram
    ...
    G-->>E: снимок рынков вселенной
    H-->>E: снимок рынков вселенной
    ...
    E->>E: непрерывность, ворота снабжения, сторож залога, стоп по просадке
    alt запас до ликвидации тонкий или отдано два круга от пика
...
- **Вселенная собирается живым отбором.** Все бессрочные рынки GMX в Arbitrum и Avalanche минус
  отсечённые воротами отбора; на снимке 18.09.2026 это 50 рынков из 151. Плюс пять зашитых имён
  запаса: двуногие ETH и BTC (GMX V2 на Arbitrum против Hyperliquid) и одноногие ETH-Arb, BTC-Arb
  (Arbitrum) и ETH-Avax (Avalanche), где нога одна: короткая на GMX с залогом в самом активе, и при
  плече 1 залог сам служит противовесом. Состав пересобирается своим кадансом и не меняется внутри
  одного решения. Для правила входа все рынки вселенной равноправны.
...
в перекладку, а каждое такое решение стоит круга издержек ($6.25-$6.90 при $2500). Поэтому ворота
...
Боту всё равно, куда входить. Все рынки вселенной, двуногие и одноногие, на Arbitrum и на
...
%%{init: {"themeVariables": {"fontSize": "12px"}, "flowchart": {"nodeSpacing": 26, "rankSpacing": 18, "diagramPadding": 4, "wrappingWidth": 380}}}%%
flowchart TD
    A["Вселенная: отбор, около 50 рынков"] --> B{{"Ворота снабжения: 720 строк истории,<br/>база известна в 684 ч?"}}
...
константа: 0.21% ноционала плюс $1 у двуногой схемы (0.12% плюс $1 у одноногой), плюс измеренный
удар по глубине GMX, плюс измеренное проскальзывание стакана Hyperliquid при этом размере. Без
обоих измерений это $2.05 при $500, $5.20 при $2000, $6.25 при $2500; удар GMX на живом составе
18.09 добавлял при $2500 от нуля до 2.6 базисного пункта, то есть не больше $0.65 сверху. Коды отказа правила размера, каждый достижим и виден в сводке: `no_capital_cap`,
`horizon_missing`, `src_gmx_down`, `src_hl_down`, `src_implausible`, `no_base`, `stale_base`,
`zero_base`, `base_identity_broken`, `no_book`, `stale_book`, `no_funding`, `no_room`,
`room_unknown`, `below_min_ticket`, `decreasing_at_every_size`, `negative_at_every_size`,
`below_fund_ratio`, `no_capital_left`. Отрицательное нетто на всей сетке это нормальный исход, а не
ошибка.

**Откуда берётся удар по глубине GMX.** Живьём он не наблюдаем вовсе: в ответах GMX нет ни одного
поля про то, как свой же объём двигает цену. Поэтому бот берёт его из измеренного снимка глубины,
который лежит внутри приложения: 63 рынка Arbitrum, 623 256 исполненных сделок за год. Рынок,
которого в снимке нет (другая цепь либо листинг новее снимка), получает кривую своего яруса
ликвидности, и источник кривой пишется у каждой строки решения, чтобы подстановка чужой кривой
никогда не была молчаливой. У снимка есть срок годности, 180 суток от конца его периода, для
нынешнего это 17.12.2026. После срока расчёт не меняется, потому что обе замены измерены и обе
хуже, но возраст и просрочка едут в запись решения, а при старте бот говорит о них строкой в
журнале. Статья «проскальзывание GMX» в настройках круга применяется ТОЛЬКО к рынку без кривой:
когда кривая есть, правило берёт измеренное вместо настроенного.

Три из этих кодов появились после аудита механики 18.09.2026, и все три про одно: неизвестность
обязана называться, а не подменяться удобным значением. `room_unknown` это «места на рынке не
наблюдали вовсе», и раньше такой рынок читался как рынок с бесконечной ёмкостью. `zero_base` это
наблюдённый ноль базы фандинга, который раньше было не отличить от непроверенного тождества.
`src_implausible` это «источник ответил, но его числа не сходятся сами с собой». Отдельно стоит
`held_missing` в журнале автомата: рынка, на котором мы стоим, в срезе нет вовсе, и правило выхода
при этом не зовётся вслепую.
...
годности решения 72 ч, требуемое покрытие баз 95%, стоп по просадке 2 круга. Каданс замерен: 24 часа
дают то же нетто, что 1 час, за 27 кругов вместо 44; стоп замерен на трёх периодах истории (раздел
«Стоп по просадке»). Срок годности и покрытие названы в коде допущениями. Правка
...
Что при этом видно. Карточка «Последняя оценка по рынкам»: строка на каждый рынок вселенной с
...
### Виджет расчёта входа

Карточка «Расчёт входа» показывает ход текущего цикла: проверку данных, обход вариантов,
подбор размера, рейтинг и результат решения. Обновления приходят из самого правила размера:
начало рынка, проверяемый размер и завершённая кривая. Между циклами карточка показывает
сохранённый расчёт; опрос раз в пять минут сам по себе не запускает новый перебор.

Живой перебор семи схем занимает доли секунды, и глазом его не увидеть. Кнопка «Повторить
расчёт» проигрывает записанные движком проверки размеров того же цикла с паузами для показа: те
же числа в том же порядке, ничего не пересчитывается, а статус карточки на время повтора
меняется на «повтор расчёта». Живое обновление трассы повтор прерывает.

В таблице видны семь схем. Два направления, не выбранные по средней ставке за окно, помечены
отдельно: полный расчёт размера и нетто для них не выполнялся. Для остальных показаны полученные движком
размер, брутто, круг издержек, нетто и исход проверки. После обхода годные варианты расположены
по рангу движка. Лучший кандидат ещё не означает вход: карточка различает выбор, отказ сторожа
и фактически открытую бумажную позицию.

При входе расчёт сохраняется с позицией и остаётся в карточке до её закрытия, в том числе после
перезапуска. Суточная проверка удержания не заменяет исходное обоснование входа. При закрытии
виден итог сделки; следующий вход закрепляет уже свой расчёт. Для старых позиций без такого
снимка история расчёта не восстанавливается из текущих рыночных данных.
...
конфигурация выбрана, решает средняя ставка за окно оценки: та, где котируемое нетто за окно выше.
...
- **Фандинг Hyperliquid** начисляется дискретно, один раз на каждую пройденную границу часа, в
  любую сторону, разбавлением не трогается: наш вход в базу GMX его не меняет. Ставка границы это
  расчётная ставка биржи за этот час (`fundingHistory`), а если биржа ещё не отдала строку, ставка
  последнего снимка до границы. Снимок через минуты после границы это прогноз следующего часа, и по
  нему леджер больше не считает: замер 05.09.2026 на 62 границах живой сделки показал, что так нога
  HL занижалась на 8% и получала ложные отрицательные часы. Источник ставки назван в описании
  строки журнала операций.

Ещё одна поправка ноги Hyperliquid появилась после аудита 18.09.2026. Биржа платит по ТЕКУЩЕЙ
стоимости позиции, а леджер книжил ногу на ноционал ВХОДА, то есть на сумму, которая после хода цены
уже не соответствует действительности. Теперь начисление умножается на отношение текущей марковой
цены к цене входа. На закрытой сделке BTC пересчёт дал $7.8559 вместо $7.7760, то есть около
процента ноги при ходе цены 1%; на монетах расширенной вселенной, где цена за сделку ходит на
десятки процентов, поправка того же порядка, что и сам ход. Цена берётся марковая, потому что
оракульной в приложении нет вовсе; остаточная разница между ними измерена и лежит в десятых долях
базисного пункта.
...
Правило выхода реализованный результат сделки не видит: оно сравнивает брутто окна назад, нетто
лучшей альтернативы и ноль кэша, а уже уплаченный круг и накопленная прибыль в это сравнение не
входят. Отсюда следствие, замеренное на живом кадре: при развороте ставок сделка держится, пока
среднее за 720 часов не уйдёт в минус, и отдаёт рынку примерно всё брутто окна при любой скорости
разворота. Стоп по просадке это единственное место, где реализованный результат участвует в
поведении, и стоит он рядом с правилом, а не внутри него.

Каждый тик сторож читает у леджера самой сделки три числа: накопленный брутто с входа, его пик и
круг издержек по модели (вход списан при открытии, выход спишется при закрытии). Когда пик минус накопленное достигает двух кругов, сделка
закрывается на этом же тике (`drawdown_stop`), не дожидаясь каданса, и следующее решение приходит
через каданс: немедленный перезаход в тот же рынок по тому же трейлингу был бы спором сторожа с
правилом входа. По приоритету стоп стоит ниже сторожа залога (ликвидация ноги невосстановима, а
отданные деньги уже отданы) и выше ворот снабжения: ему не нужны ни кадр, ни базы, поэтому дыра в
базах удерживаемого рынка ему не мешает. Числа леджера, которых нет, стоп не зовут: это отказ
снабжения, а не просадка.

Порог замерен, а не назначен. Стенд `exit-6-loss.mjs` прогнал семь кандидатов поведения в
убыточной сделке на трёх периодах истории при капитале $2500: год из 63 рынков, второй период из
22 и длинное окно из 14. Стоп в два круга единственный выиграл у правила везде: на одном и том же
старте он даёт на $4.4, $12.5 и $10.8 больше при 40 победах из 40 стартов на каждом периоде и при
том же числе кругов; без любого из 63 имён вывод держится в 58 случаях из 63. Стоп в один круг
лучший на первом году и проигрывает вне выборки, стоп в полкруга, полоса отрицательных часов
короче двух суток, экспоненциальное окно и схлопывание потока теряют от $34 до $1290 лишними
кругами. Срабатывает стоп один-три раза в год: на сделках, отдавших два круга, правило держало бы
позицию ещё 100..200 часов после пика её результата. Цена стопа в деньгах невелика, порядка $5..15
в год при $2500, и подбирать порог дальше нельзя: сетка вокруг двух кругов дала бы число,
подогнанное под те же три периода. На карточке честности стоп показан строкой «отдано от пика
накопленного при пороге»; параметр замораживается взводом вместе с остальными.
...
- `fa-dec-<день>.ndjson` - расчёт решения на каждой точке решения: возраст данных, капитал, пресет,
  окно трейлинга, кривые всех профинансированных рынков, все отказы, блок правила выхода. Здесь же
  снимок глубины GMX, по которому посчитан круг: дата конца его периода, возраст на час решения и
  признак просрочки, а у каждого рынка и каждого отказа имя источника его кривой (своя кривая
  рынка, ярус ликвидности, общий пул, нет вовсе). Без них круг издержек в строке нечем объяснить
  задним числом.
...
Объём замерен дважды. На пяти зашитых рынках строка снимка с позицией около 2.2 КБ, при опросе раз
в 5 минут это 0.63 МБ в сутки и 0.23 ГБ в год. На живой вселенной из 53 инструментов замер на
работающей машине дал 5.72 МБ в сутки, то есть около 2.09 ГБ в год. Снимки составляют больше 99%
объёма. Срок хранения бесконечен: приложение не умеет удалять записи, и канала на удаление нет.
...
- **Честность счёта**, четыре замера, и каждый означает, что показанная прибыль больше настоящей:
  ...
  строкой «показанный P&L этого риска не содержит»; строка «вне выборки край не доказан»: второй период из 22 крупных имён даёт правилу $712 в
  год против $413 у «вошли и держим», малых имён там нет.
- **Затухание фандинга GMX** на карточке честности: ставка своей стороны сейчас, сколько часов
  подряд она убывает и с каким шагом, час нуля по тренду и перекос открытого интереса. Это
  наблюдение, а не решение: правило выхода и сторожа этих чисел не видят, потому что на трёх
  периодах истории закрытие по затуханию проигрывает правилу кругами (замер З7, 05.09.2026).
- **Зона Ⅱ · Торговля**: P&L счёта с момента запуска (у открытой сделки «если закрыть сейчас»:
  брутто минус вход минус выход по модели; у закрытой «реализовано · нетто»), уплачено на входе и
  выход по модели порознь, таблица позиций, параметры выбранной позиции, форвардная кривая
  капитала с t0, журнал операций с выгрузкой CSV, XLSX и JSON. Круг издержек разнесён
  соглашением: вход списывается строкой при открытии, выход строкой при закрытии, а пока сделка
  открыта, выход по модели стоит под итогами журнала и в суммы не входит.
...
## Тик с открытой сделкой: сторожа, события, каданс

Правило выхода зовётся не каждый тик, а сторожа работают каждый. Порядок ниже это порядок
приоритетов кода: то, что стоит выше, решает раньше, и его отказ виден в журнале, даже когда решил
не он. Перерыв опроса и первый тик после запуска блокируют всё: сторожа на таком тике считаются и
показываются, но закрывают на следующем непрерывном тике. Дыра в базах рынка сделки блокирует только
правило: сторож залога и стоп по просадке считают по цене и леджеру и в кадре не нуждаются.

```mermaid
%%{init: {"themeVariables": {"fontSize": "12px"}, "flowchart": {"nodeSpacing": 26, "rankSpacing": 18, "diagramPadding": 4, "wrappingWidth": 380}}}%%
flowchart TD
    T0["Тик каждые 5 минут при открытой сделке:<br/>сначала начисление леджера"] --> T1{{"Перерыв опроса, первый тик после запуска,<br/>тумблер выключен, состояние старше 72 часов?"}}
    T1 -- "да" --> S1["Ничего не решается; сторожа считаются<br/>и показываются, закрытие на следующем непрерывном тике"]
    T1 -- "нет" --> T2{{"Сторож залога: запас худшей ноги<br/>ниже требуемых 50%?"}}
    T2 -- "нет цены или плеча" --> S2["margin_unknown: ждём данных, закрытия нет"]
    T2 -- "да" --> X1["margin_thin: закрыть"]
    T2 -- "нет" --> T3{{"Стоп по просадке: пик накопленного<br/>минус накопленное не меньше двух кругов?"}}
    T3 -- "да" --> X2["drawdown_stop: закрыть,<br/>следующее решение через каданс"]
    T3 -- "нет" --> T4{{"Ворота рынка сделки: 720 часов ставок,<br/>базы не меньше 684 часов?"}}
    T4 -- "нет" --> S3["hist_short или hist_no_base: правило не зовётся,<br/>позиция держится, сторожа работают дальше"]
    T4 -- "да" --> T5{{"Событие или каданс?"}}
    T5 -- "ставка против нас 6 часов подряд" --> R
    T5 -- "поток рынка упал вдвое" --> R
    T5 -- "запас сжался на 10 пунктов" --> R
    T5 -- "24 часа с прошлого решения" --> R
    T5 -- "ничего" --> S4["cadence_wait: держим"]
    R["Правило выхода: максимум из брутто удержания,<br/>нуля кэша и нетто лучшей альтернативы<br/>(диаграмма в следующем разделе)"]
```
...
не может. Единственное место, где реализованный итог участвует в поведении, это стоп по просадке,
и он стоит рядом с правилом, а не внутри него (раздел «Стоп по просадке»). Кэш это альтернатива с нетто ноль, а не отдельная ветка, поэтому порядка проверок,
...
Закрытие руками сделки, которую вёл сам автомат, тоже разрешено и с версии 0.4.0 больше его не
заклинивает. Раньше указатель слота оставался стоять на закрытой сделке, автомат её не находил и
вставал навсегда с кодом `orphan_position`, а перевзвод из интерфейса не лечил и вредил. Теперь
указатель лечится на месте: если сделка в леджере лежит закрытой, слот считается свободным, тик
продолжается, а в журнале рядом с кодом стоит признак «вылечено». Случай, когда сделки в леджере нет
вовсе, по-прежнему останавливает автомат: это уже не закрытие, а потеря состояния. Само ручное
закрытие пишет в архив сделок строку `close` с причиной `manual_close`, чтобы действие человека
нельзя было спутать с выводом правила.
...
flowchart LR
    ...
    C["Круг издержек: 0.21% плюс $1<br/>плюс удар GMX и проскальзывание стакана,<br/>один раз при входе"] --> N
```
...
- вне выборки, на втором периоде проекта (22 крупных имени, 2024-12..2025-06), правило даёт $712 в
  год против $413 у простого «вошли и держим» при потолке тикета, приведённом к капиталу, как у
  автомата; прежняя запись «$214.80 против $429.99, база бьёт правило вдвое» была свойством стенда
  (потолок $5000 при капитале $2500: рынок назначения выбирался при чужом размере) и опровергнута
  4 сентября 2026; оговорка остаётся: в том периоде нет малых имён, на которых первый год и
  заработал, и воспроизводимость края по вселенной этим не доказана;
...
## Карта убытков: что может отнять деньги и что бот с этим делает

Каждая ветка ниже это способ потерять деньги в открытой сделке, и у каждой назван ответ кода.
Зелёный: ветка есть в коде и под тестом. Жёлтый: бот реагирует, но с оговоркой (лаг окна, порог
события выбран, а не замерен, ликвидация по модели, а не по бирже). Красный: ни леджер, ни правило
этого не видят по построению, и лечится это не кодом решения, а размером, плечом и порядком действий
на настоящем счёте.

```mermaid
%%{init: {"themeVariables": {"fontSize": "12px"}, "flowchart": {"nodeSpacing": 14, "rankSpacing": 36, "diagramPadding": 4, "wrappingWidth": 320}}}%%
flowchart LR
    L["Убыток в открытой сделке:<br/>две ноги, плечо 1"]
    L --> A["Поток ставок ухудшился"]
    L --> B["Цена и маржа ног"]
    L --> C["Издержки и обороты"]
    L --> D["Данные и оценка"]
    L --> E["Исполнение на настоящем счёте"]
    A --> A1["фандинг нашей ноги GMX сменил знак, борроу вырос<br/>или ставка Hyperliquid против нас: полоса из 6 часов<br/>зовёт правило раньше каданса, стоп закрывает при отдаче<br/>двух кругов от пика, иначе в кэш только с минусом окна 720 ч"]
    A --> A2["встречная сторона ушла, поток рынка упал вдвое:<br/>событие зовёт правило раньше каданса"]
    A --> A3["разбавление собственным входом:<br/>множитель B/(B+S) в леджере и в правиле"]
    B --> B1["цена против короткой ноги Hyperliquid: событие при<br/>сжатии запаса на 10 пунктов, закрытие при запасе ниже 50%;<br/>ликвидация по модели, не по бирже"]
    B --> B2["цена против длинной ноги GMX:<br/>при плече 1 ликвидации нет"]
    B --> B3["гэп цены между тиками:<br/>сторож не успевает, леджер цену не книжит"]
    B --> B4["нет кросс-маржи между биржами:<br/>убыток одной ноги прибылью другой не покрыт"]
    B --> B5["базис марок GMX и Hyperliquid на выходе:<br/>ценовой результат не книжится"]
    C --> C1["вход по модели списан при открытии, выход при закрытии;<br/>реальный выход дороже модели не виден"]
    C --> C2["перекладка платит новый круг:<br/>требуется превышение на полный круг"]
    C --> C3["холостой оборот кэш и вход:<br/>вход при нетто выше круга, выход при брутто ниже нуля"]
    C --> C4["перекладка на разворот с лагом окна:<br/>медианное упреждение минус 54 часа"]
    D --> D1["источник молчит: решения нет,<br/>позиция висит, закрытия нет намеренно"]
    D --> D2["дыра баз на рынке сделки:<br/>правило не зовётся, сторожа работают"]
    D --> D3["состояние старше 72 часов после падения:<br/>решение заблокировано"]
    E --> E1["биржа или сеть не даёт закрыть одну ногу"]
    E --> E2["принудительное закрытие ноги биржей"]
    E --> E3["смарт-контракт, мост, депег обёрнутого актива"]
...
Красные ветки не закрываются кодом решения в принципе. Их цена на настоящем счёте задаётся
размером ноги как долей депозита, плечом 1, порядком закрытия двух ног в одно окно времени и тем,
сколько залога лежит на стороне Hyperliquid сверх ноционала: кросс-маржи между биржами нет, и
переносить залог между ними может только человек.
...
| Состав вселенной | Строка над сводкой: чем собран список (живой отбор, сохранённый файл, запас), сколько рынков просмотрено и взято, пороги отбора и перепись отказов по кодам |
| Последняя оценка по рынкам | Строка на каждый рынок вселенной: инструмент, конфигурация, ранг, исход, чем связан размер, размер, нетто за горизонт, покрытие баз, доля удержания, ставка схемы; штамп «снята · потолок капитала · следующая не раньше». При полусотне рынков есть поиск по имени, порядок по нетто, фильтр по исходу чипами с числами и свёртка: показаны первые десять и удерживаемый рынок, остальные раскрываются кнопкой |
| Расчёт входа | Последовательный обход семи схем с реальными обновлениями подбора размера, отказами и итоговым рейтингом; выбранный кандидат и открытая сделка различаются, исходный расчёт сохраняется вместе с позицией до закрытия; повтор записанного хода по кнопке |
...
| P&L счёта с t0 | У открытой сделки «если закрыть сейчас» (брутто минус вход минус выход по модели), у закрытой реализованный результат нетто; уплачено на входе и выход по модели порознь; доходность к капиталу, APR с t0 после 24 часов начисления |
...
| Журнал операций | Каждое начисление и событие позиции строкой с фильтрами и выгрузкой CSV, XLSX, JSON; издержки входа строкой при открытии, выхода при закрытии, не списанный выход подвалом под итогами |
...
| Транзакционные издержки · модель | Редактируемые статьи круга (комиссии GMX, проскальзывание GMX, газ, комиссия Hyperliquid, число сторон) на размере бота; нетто за горизонт берётся готовым из оценки правила. Статья «проскальзывание GMX» работает только у рынка без измеренной кривой удара: когда кривая есть, правило берёт измеренное вместо настроенного |
...
| `src/engine/assemble.js`, `src/engine/sources.js`, `src/engine/universe.js` | Снимки и кадры, доступ к GMX и Hyperliquid, история ставок и баз из индексатора, пять зашитых рынков запаса |
| `src/engine/fa/universe-scan.js` | Живой отбор вселенной: ворота, пороги, коды отказа, ключ рынка, разбор схемы |
| `src/engine/fa/slice.js` | Складка среза: что из вселенной и в каком виде уходит в правила |
...
| `src/engine/fa/observe.js`, `src/main/fa-entry-trace.js` | Изолированные наблюдения хода расчёта, снимки, привязка к позиции и восстановление |
| `src/renderer/entry-trace.js`, `entry-trace.css` | Виджет расчёта входа: прогресс, таблица вариантов, кривые размеров и выбранная сделка |
