...
Простое объяснение механики бота для читателя без опыта в трейдинге и опционах: что бот
делает, когда и зачем, за весь цикл своей жизни.

[English version](bot2-btc-options-how-it-works.en.md) · Документ соответствует коду на 27.08.2026.
...
```mermaid
flowchart TD
    A["Старт приложения:<br/>состояние читается с диска,<br/>открытая сделка продолжается"] --> B{"Цепочка включена<br/>и сделки нет?"}
    B -- "да" --> C["Выбор контракта:<br/>срок 14-28 дней,<br/>дельта около 0.45"]
    C --> D{"Санитария котировки:<br/>свежая, спред узкий,<br/>стакан не пустой?"}
    D -- "ни один кандидат" --> W["Ждать и пробовать снова:<br/>ретрай каждые 30 секунд;<br/>после 4 часов - открыть лучшую<br/>с постоянной пометкой"]
    W --> C
    D -- "да" --> E["Размер по стресс-правилу:<br/>пережить ход цены на 45%<br/>в любую сторону"]
    E --> F["Открытие: премия получена,<br/>противовес выставлен"]
    F --> G["В сделке. Каждые 15 секунд:<br/>подправить противовес,<br/>проверить маржу, записать журнал"]
    G --> H{"Экспирация?<br/>(08:00 UTC)"}
    H -- "нет" --> G
    H -- "да" --> I["Расчёт: итог сделки<br/>записан в цепочку"]
    I --> B
...
- **Плохие данные не рождают сделок.** Если снимок неполный (нет цены перпетуала, нет
  греков), бот пропускает решение, а не действует вслепую.
- **Опрос живёт, пока есть за чем следить.** При открытой сделке или включённой цепочке
  он стартует сам (в том числе после перезапуска); без них бот молчит и в сеть не ходит.
...
```mermaid
flowchart TD
    A["Все BTC-опционы на Deribit<br/>(сотни контрактов)"] --> B["Фильтр срока:<br/>жить осталось 14-28 дней"]
    B --> C{"Режим цепочки"}
    C -- "колл" --> D["Коллы с дельтой 0.45 ± 0.10,<br/>отсортированы по близости к 0.45"]
    C -- "стрэнгл" --> E["Пары колл + пут одной экспирации;<br/>колл выбирается тем же правилом,<br/>пут подбирается к нему"]
    E -- "пары нет вовсе" --> D
    D --> F["Санитария: до 3 кандидатов"]
    E --> F
    F --> G{"Тикер свежее 60 секунд?<br/>Спред уже 8% премии?<br/>В стакане от 5000 долларов?"}
    G -- "кандидат не прошёл" --> H["Взять следующего кандидата"]
    H --> G
    G -- "никто не прошёл" --> I["Ожидание с показанной причиной;<br/>после 4 часов - лучший кандидат<br/>с пометкой 'ухудшенная санитария'"]
    G -- "прошёл" --> J["Размер: стресс-правило ±45%,<br/>резерв не выше 80% счёта,<br/>залог входа не больше счёта"]
    I --> J
    J --> K["Сделка открыта:<br/>премия и издержки входа в журнале"]
```
...
что решает полоса, а не частота проверок. (У ручного четырёхногого режима триггеры
времени и цены и фильтр выгоды есть и настраиваются в тулбаре; у схемы продавца они
выключены намеренно - так она измерена.) Подправка моделируется лимитной заявкой по
середине спреда (комиссия мейкера 0%); если оператор выбрал «маркет» - через спред с
комиссией тейкера. У перпетуала есть фандинг - небольшие периодические платежи между
покупателями и продавцами; бот начисляет его каждый тик, в любую сторону.

**Выходов до экспирации нет.** Ни стопов, ни тейк-профитов: вся статистика схемы снята
с единственным выходом «дожить до экспирации», а испытания досрочных выходов в проекте
не дали ни одной прибыльной конфигурации. Закрыть сделку руками можно, но она навсегда помечается как
закрытая досрочно и отделяется от измеренной статистики.
...
Стресс-правило размера (раздел выше) и маржинная тревога - две части одной защиты:
первая не даёт взять опасный размер на входе, вторая следит за риском по ходу сделки.
...
stateDiagram-v2
    ...
    Open --> Off: ручное закрытие (помечается)
```
...
запуске и помечается как поздний.
...
timeline
    ...
    День 0 : Прошлая сделка рассчитана в 08.00 UTC : Через полминуты выбран и продан новый стрэнгл : Противовес выставлен
    Дни 1-20 : Тик каждые 15 секунд : Подправки противовеса по полосе : Маржа под присмотром
    День 21 : Расчёт в 08.00 UTC по индексу : Итог и доходность на залог в журнале : Сразу открывается следующее звено
```
...
| Журнал (леджер) | Каждое событие строкой: открытие, издержки входа, хеджи, разрывы фандинга, тревоги маржи, расчёт, поправка delivery; выгрузка CSV, XLSX, JSON |
| Паспорт соответствия | Построчная сверка замороженной конфигурации сделки с измеренной схемой: полоса, триггеры, блэкаут, тенор, правило размера, выход |
...
| График выплат | Профиль «что будет к экспирации при цене X» с отметками входа, текущей цены и LIQ |
...
- **Не ставит стопы и тейки.** Единственный измеренный выход - дожить до экспирации;
  досрочные выходы не дали ни одной прибыльной конфигурации на истории.
