...
3. **Издержки хеджа.** Каждое бумажное исполнение перпетуала списывает комиссию тейкера — плату за сделку, исполненную сразу по рыночной цене, — 0.05% от суммы сделки; покупка исполняется по цене продавца, продажа по цене покупателя, то есть спред пересекается каждый раз. Убыток опционов ограничен дебетом, а издержки количеством сделок не ограничены. Именно поэтому в движке стоят дедбэнд и фильтр `λ`: они не дают хеджу «пилить» депозит мелкими сделками.
...
- Решение **«ПРОПУСК»** на большинстве циклов — норма, а не поломка. Полный список причин пропуска — в разделе 5; на размере 0.01 чаще всего срабатывает округление: хедж исполняется целыми контрактами перпетуала по $10, и требуемая поправка часто округляется до нуля. Каждый пропуск — это сэкономленные комиссия и спред.
...
Первое и главное правило: **всегда закрывайте структуру до экспирации**, лучше до начала 30-минутного блэкаута. Симулятор не рассчитывает исполнение опционов на экспирации: если позицию не закрыть, ноги исчезнут из рыночных данных, гейт греков перестанет проходить и переоценка замрет. Формулировки шага 0 про «к экспирации» описывают форму выплаты, а не совет держать до конца — результат фиксирует только закрытие.
...
- **Штиль при близкой экспирации.** Если цена стоит у центрального страйка, каждый час аренды приближает потерю всего дебета. Досрочное закрытие спасает остаток премии, который еще не съела тета.
- **Блэкаут — не «если», а «раньше»:** в последние 30 минут до экспирации движок не хеджирует, поэтому выходить нужно до его начала.
...
2. **Запишите значения карточки «Метрики прогона»** (или сделайте скриншот) до следующего открытия — экспорт журнала их не содержит, а при открытии новой структуры карточка обнуляется. Смотрите на долю прибыльных циклов, просадку и число хеджей. Sharpe здесь считается по циклам и без аннуализации — почему его нельзя сравнивать с дневным или годовым Sharpe других стратегий, объясняет раздел 6.
