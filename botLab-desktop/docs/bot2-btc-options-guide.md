...
5. **Решение о хедже.** Принимается только при открытой структуре, оценённом фьючерсе и полном наборе данных по всем ногам («гейт греков»):
   - включён блэкаут и идёт его окно (07:50–08:10 UTC или последние 30 минут перед экспирацией) → решение **«ПАУЗА»**, сделок нет;
   - иначе проверяются три триггера: общая дельта вышла за допустимый коридор (дедбэнд); цена BTC изменилась с последнего хеджа не меньше порога; с последнего хеджа прошло 60 секунд и больше. Ни один не сработал → **«ПРОПУСК»**;
   - размер сделки — сколько фьючерса купить или продать, чтобы вернуть дельту к нулю, — округляется до целых контрактов по $10. Ноль после округления → **«ПРОПУСК»**;
   - расчётная польза сделки сравнивается с издержками (комиссия, спред, проскальзывание, ожидаемое финансирование), умноженными на запас λ. Не превышает → **«ПРОПУСК»**; превышает → **«ХЕДЖ»**.
...
Дальше каждый шаг разобран подробно: интерфейс и статусы данных (разделы 1–2), параметры (раздел 3), открытие (раздел 4), решения движка (раздел 5), аналитика и свип (разделы 6–7), журнал и закрытие (раздел 8), ограничения (раздел 9), практикум одной сделки (раздел 10).
...
*Полный жизненный цикл одной сделки в журнале: `открытие → хедж → … → закрытие перпа → фиксация опционов`. Статус **«✓ сходится»** означает, что компоненты P&L арифметически сходятся к итогу. Разберите: сколько было хеджей и сколько ушло на комиссии (экспорт всегда полный — CSV / XLSX / JSON).*
