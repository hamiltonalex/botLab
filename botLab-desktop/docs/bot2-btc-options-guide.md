...
3. Показывает тикет: цена входа (нетто-дебет), максимальные убыток и прибыль, состояние данных, результаты проверок. Блокирующие причины — неверный размер, несовпадение ног, блэкаут — запрещают открытие; предупреждение `IM > депозит` (расчётный залог биржи больше счёта) — нет.
