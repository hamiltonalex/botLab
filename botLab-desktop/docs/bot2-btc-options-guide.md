...
- [Как работает бот: механика по шагам](#как-работает-бот-механика-по-шагам)
  - [С чего бот начинает](#с-чего-бот-начинает)
  - [Какие данные бот собирает](#какие-данные-бот-собирает)
  - [Что бот делает на каждом свежем снимке](#что-бот-делает-на-каждом-свежем-снимке)
  - [Как открывается позиция](#как-открывается-позиция)
  - [Как закрывается позиция](#как-закрывается-позиция)
  - [Если с данными проблемы](#если-с-данными-проблемы)
  - [Что можно менять во время работы и как это влияет](#что-можно-менять-во-время-работы-и-как-это-влияет)
- [О модуле: стратегия и интерфейс](#о-модуле-стратегия-и-интерфейс)
- [1. Быстрый старт](#1-быстрый-старт)
- [2. Рыночные данные Deribit](#2-рыночные-данные-deribit)
- [3. Параметры конструктора](#3-параметры-конструктора)
  - [Сигнал IV](#сигнал-iv)
  - [Превью и издержки](#превью-и-издержки)
- [4. Открытие структуры](#4-открытие-структуры)
- [5. Открытая позиция и решения движка](#5-открытая-позиция-и-решения-движка)
- [6. Аналитика](#6-аналитика)
- [7. Свип параметров](#7-свип-параметров)
- [8. Журнал и закрытие](#8-журнал-и-закрытие)
- [9. Файлы и ограничения](#9-файлы-и-ограничения)
- [10. Практикум: одна сделка от начала до конца](#10-практикум-одна-сделка-от-начала-до-конца)
  - [Шаг 0. Поймите, на чём вообще может вырасти бумажный депозит](#шаг-0-поймите-на-чём-вообще-может-вырасти-бумажный-депозит)
  - [Шаг 1. Запустите данные и дождитесь LIVE](#шаг-1-запустите-данные-и-дождитесь-live)
  - [Шаг 2. Оцените сигнал IV](#шаг-2-оцените-сигнал-iv)
  - [Шаг 3. Выберите размер и посмотрите на маржу честно](#шаг-3-выберите-размер-и-посмотрите-на-маржу-честно)
  - [Шаг 4. Откройте тикет и проверьте всё до подтверждения](#шаг-4-откройте-тикет-и-проверьте-всё-до-подтверждения)
  - [Шаг 5. Наблюдайте за движком, не мешая ему](#шаг-5-наблюдайте-за-движком-не-мешая-ему)
  - [Шаг 6. Закройте сделку осознанно](#шаг-6-закройте-сделку-осознанно)
  - [После сделки: журнал, метрики, свип](#после-сделки-журнал-метрики-свип)
  - [Чего практикум не обещает](#чего-практикум-не-обещает)

---

## Как работает бот: механика по шагам

Бот торгует опционами на BTC в режиме тренажёра: цены и данные - настоящие, с биржи Deribit; деньги - виртуальные (депозит $100), реальные ордера не отправляются. Опцион - это контракт, дающий право купить («колл») или продать («пут») BTC по заранее зафиксированной цене - страйку - до назначенной даты, экспирации. Позиция бота всегда состоит из двух частей:

- **структура** - четыре опциона с одной экспирацией: два купленных со страйком у текущей цены BTC и два проданных дальше от неё. Связка приносит прибыль при сильном движении BTC в любую сторону и теряет уплаченную премию, если цена остаётся на месте;
- **хедж** - противоположная позиция во фьючерсе BTC-PERPETUAL (контракте, повторяющем цену BTC). Ею бот гасит дельту - текущую зависимость результата от направления цены, - чтобы итог определялся размахом колебаний, а не угаданным направлением.

Ниже вся механика показана тремя схемами. Первая - общая карта: как позиция проходит путь от запуска приложения до закрытия сделки и что на каждом переходе делает пользователь, а что бот сам.

```mermaid
stateDiagram-v2
    direction TB
    state "Запуск приложения" as S0
    state "Опрос данных выключен" as S1
    state "Готов · открытой позиции нет" as S2
    state "Позиция открыта" as S3

    [*] --> S0
    S0 --> S1 : восстановление позиции, журнала и итога с диска
    S1 --> S2 : пользователь включает опрос данных
    S2 --> S2 : авто · каждый Реприс · сбор данных и IV-сигнал (сам не открывает)
    S2 --> S3 : пользователь · Старт-авто или Открыть структуру
    S3 --> S3 : авто · каждый Реприс · переоценка и решение о хедже
    S3 --> S2 : пользователь · двойной клик «закрыть структуру» (нужен LIVE)
    S3 --> S2 : авто · экспирация · расчёт по индексу BTC
```

*Переходы с пометкой «пользователь ·» запускает человек: открыть структуру и досрочно закрыть её можно только вручную. Пометкой «авто ·» отмечено то, что бот делает сам: хеджирует на каждом снимке и рассчитывает позицию на экспирации (единственный автоматический выход - стоп-лоссов и ликвидации в симуляторе нет). Петли у состояний - это повторяющийся цикл опроса. Итог не обнуляется: следующая позиция открывается на «$100 плюс весь накопленный P&L».*

### С чего бот начинает

1. При запуске приложения бот только восстанавливает с диска прошлое состояние: открытую позицию, журнал операций, накопленный итог и историю волатильности. Опрос биржи выключен; сам бот ничего не открывает.
2. Опрос данных включает пользователь: кнопкой источника («НЕТ ДАННЫХ»), кнопкой «Пересчёт» или открытием структуры.

### Какие данные бот собирает

Каждые несколько секунд - интервал задаёт селектор «Реприс» (5/15/30 с) - бот запрашивает у Deribit:

1. цену, лучшие заявки и ставку финансирования фьючерса BTC-PERPETUAL (ставка финансирования - периодический платёж между сторонами фьючерса, для бота доход или расход);
2. цены и греки опционов - готовые, рассчитанные биржей чувствительности к цене BTC, волатильности и времени; бот сам опционы не оценивает. Опрашиваются ноги открытой структуры и дежурный набор опционов вокруг текущей цены;
3. раз в 5 минут - список доступных опционов и индекс волатильности DVOL (при первом запросе - его историю за 48 часов).

Из снимков копятся две истории: подразумеваемой волатильности (точка не чаще раза в 30 с, окно 24 ч, сохраняется на диск) и кольцо последних 600 сырых снимков рынка для свипа (живёт до перезапуска приложения).

### Что бот делает на каждом свежем снимке

1. **Расчёт экспирации.** Если срок опционов открытой структуры истёк, бот рассчитывает её сам: закрывает фьючерс по рынку, засчитывает опционам итоговую стоимость по текущему индексу BTC, пишет событие в журнал и очищает позицию. Если приложение было выключено в момент экспирации, расчёт произойдёт на первом же снимке после запуска - с пометкой об опоздании.
2. **Финансирование.** Начисляет платёж по фьючерсу за время с прошлого снимка (после перерыва в данных - не более чем за 300 секунд).
3. **Переоценка.** Пересчитывает по свежим ценам стоимость позиции, P&L, общую дельту, маржу, метрики прогона и стресс-сценарии.
4. **Сигнал входа.** Обновляет IV-ранг - положение текущей волатильности в диапазоне последних суток (0 - минимум, 1 - максимум). Ранг ≤ 0.35 подсвечивается как благоприятный для входа. Сигнал только информирует: сам бот позицию не открывает.
5. **Решение о хедже.** Принимается только при открытой структуре, оценённом фьючерсе и полном наборе данных по всем ногам («гейт греков»):
   - если включён блэкаут и идёт его окно (07:50–08:10 UTC или последние 30 минут перед экспирацией), движок возвращает **«ПАУЗА»**, сделок нет;
   - иначе проверяются три триггера: общая дельта вышла за допустимый коридор (дедбэнд); цена BTC изменилась с последнего хеджа не меньше порога; с последнего хеджа прошло 60 секунд и больше. Если не сработал ни один, движок возвращает **«ПРОПУСК»**;
   - размер сделки - сколько фьючерса купить или продать, чтобы вернуть дельту к нулю, - округляется до целых контрактов по $10. Если после округления получается ноль, движок возвращает **«ПРОПУСК»**;
   - расчётная польза сделки сравнивается с издержками (комиссия, спред, проскальзывание, ожидаемое финансирование), умноженными на запас λ. Если польза не превышает эти издержки, движок возвращает **«ПРОПУСК»**, а если превышает, то **«ХЕДЖ»**.
6. **Исполнение «ХЕДЖ»** - виртуальная сделка: стиль «маркет» исполняется по цене встречной заявки с комиссией 0.05%, стиль «лимит» - по середине спреда без комиссии. Сделка записывается в журнал.
7. **Сохранение.** Состояние (позиция, журнал, P&L) пишется на диск после каждого снимка - приложение можно закрыть в любой момент.

Пункты 1–7 выше - это один проход по свежему снимку. Та же последовательность в виде схемы:

```mermaid
flowchart TD
    T["Таймер источника · интервал = «Реприс» (5 / 15 / 30 с)"] --> Q["Запрос к Deribit: перп BTC-PERPETUAL (цена, заявки, финансирование) и греки ног структуры плюс дежурный набор опционов ATM ± 5/10/15 %"]
    Q --> U["Раз в 5 минут: список доступных опционов и индекс волатильности DVOL (при первом запросе - история за 48 ч)"]
    U --> H["Запись историй: IV (не чаще 1 точки за 30 с, окно 24 ч, на диск) и кольцо из 600 сырых снимков"]
    H --> F["Начисление финансирования на открытый перп (после паузы в данных - не более чем за 300 с)"]
    F --> E{"Срок опционов открытой структуры истёк?"}
    E -->|да| S["Авто-расчёт: закрыть перп по рынку, зачесть опционам итоговую стоимость по индексу BTC, событие в журнал, очистить позицию"]
    E -->|нет| R["Переоценка: стоимость позиции, P&L, общая дельта, маржа, метрики прогона, стресс-сценарии"]
    S --> R
    R --> IV["Сигнал входа · IV-ранг за сутки: 0 минимум … 1 максимум; ранг не больше 0.35 - благоприятно. Только информирует, сам не открывает"]
    IV --> D["Решение о хедже - см. следующую схему"]
    D --> SV["Сохранение состояния на диск и обновление интерфейса"]
    SV -->|следующий снимок| T
```

*Проход повторяется каждые 5/15/30 секунд, пока приходят свежие данные (статус LIVE). Финансирование начисляется в начале обработки снимка, ещё до проверки экспирации; расчёт на экспирации и хедж - автоматические, а IV-ранг только подсвечивает вход, не открывая позицию сам. Семь пунктов списка выше перечисляют те же действия.*

Шаг «Решение о хедже» (пункт 5) - это отдельная последовательность проверок. Бот проходит их сверху вниз и на каждом снимке выдаёт ровно один исход:

```mermaid
flowchart TD
    A(["Свежий снимок при открытой структуре"]) --> G{"Структура открыта, перп оценён и греки всех ног полны?"}
    G -->|"нет · гейт греков"| GP["Статус «ПРЕДУПР.» · новые хеджи не создаются, финансирование продолжает начисляться"]:::warn
    G -->|да| B{"Блэкаут включён и идёт его окно?<br/>07:50–08:10 UTC или последние 30 мин перед экспирацией"}
    B -->|да| PA["«ПАУЗА» · сделок нет"]:::pause
    B -->|нет| TR{"Сработал хотя бы один триггер?<br/>дельта вышла за дедбэнд · цена сдвинулась не меньше порога · с последнего хеджа прошло 60 с и больше"}
    TR -->|"ни один"| SK1["«ПРОПУСК» · ордер не требуется, edge сохранён"]:::skip
    TR -->|да| SZ{"Размер хеджа после округления до целых контрактов по 10 долларов равен нулю?"}
    SZ -->|да| SK2["«ПРОПУСК»"]:::skip
    SZ -->|нет| CB{"Польза больше, чем издержки, умноженные на λ?<br/>издержки = комиссия плюс спред плюс проскальзывание плюс ожидаемое финансирование"}
    CB -->|нет| SK3["«ПРОПУСК»"]:::skip
    CB -->|да| HG["«ХЕДЖ» · виртуальная сделка"]:::hedge
    HG --> EX["Маркет: по встречной заявке, комиссия 0.05%. Лимит: по середине спреда, без комиссии. Запись в журнал, дельта возвращается к нулю"]:::hedge

    classDef hedge fill:#2ea04322,stroke:#2ea043,color:#2ea043
    classDef skip fill:#8b949e22,stroke:#8b949e,color:#8b949e
    classDef pause fill:#d2992222,stroke:#d29922,color:#d29922
    classDef warn fill:#d2992222,stroke:#d29922,color:#d29922
```

*Зелёный - «ХЕДЖ» (уходит виртуальный ордер); серый - «ПРОПУСК» (повод есть, но фильтр не пройден); амбер - «ПАУЗА» (окно блэкаута) и «ПРЕДУПР.» (гейт греков не пройден). Само решение бот принимает автоматически. Пороги - дедбэнд, ценовой триггер, λ, стиль исполнения, блэкаут - задаёт пользователь, и при открытии они замораживаются внутри позиции; временной триггер 60 с фиксирован и из интерфейса не меняется.*

### Как открывается позиция

1. Открытие всегда запускает пользователь. **«Старт (авто)»** - бот сам берёт ближайшую экспирацию в пределах трёх дней, до которой осталось больше 30 минут; **«Открыть структуру»** - использует выбранную в конструкторе.
2. Бот выбирает страйки: для купленных колла и пута - ближайший к текущей цене BTC, для проданных - ближайшие к цене ± выбранный офсет. Затем запрашивает свежие котировки всех четырёх ног.
3. Показывает тикет: цена входа (нетто-дебет), максимальные убыток и прибыль, состояние данных, результаты проверок. Блокирующие причины - неверный размер, несовпадение ног, нога без котировки, блэкаут - запрещают открытие; предупреждение `IM > депозит` (расчётный залог биржи больше счёта) - нет.
4. После подтверждения ноги фиксируются по текущим ценам, параметры движка замораживаются внутри позиции, начинается цикл хеджирования.

### Как закрывается позиция

- **Пользователем:** двойное нажатие **«закрыть структуру»** - фьючерс закрывается по рынку с комиссией, результат опционов фиксируется по текущей переоценке. Закрытие требует свежих данных: по устаревшему снимку или без цены фьючерса бот не закрывает, а просит дождаться статуса «LIVE».
- **Автоматически - только на экспирации** (шаг 1 цикла). Других автоматических выходов нет: стоп-лоссов и принудительной ликвидации в симуляторе не существует, при любой просадке позиция живёт до экспирации или ручного закрытия.
- Итог не обнуляется: следующая позиция открывается на счёт «$100 плюс весь накопленный P&L».

### Если с данными проблемы

- Не хватает цены или греков по ноге открытой структуры (гейт греков не пройден, статус «ПРЕДУПР.»): новые хеджи не создаются, недостающие ноги переоцениваются по ценам входа, финансирование продолжает начисляться.
- Свежие снимки перестали приходить (статус «УСТАРЕЛО»): цикл стоит, решения не принимаются до следующего снимка.
- После перезапуска приложения позиция, журнал и итог на месте, но опрос данных нужно включить заново.

### Что можно менять во время работы и как это влияет

- **Действует сразу, в том числе на открытую позицию:** интервал «Реприс» - меняет частоту всего цикла, то есть как часто бот пересматривает решения. Повторное нажатие кнопки источника - внеочередное обновление данных.
- **Сохраняется сразу, но применяется со следующего открытия:** дедбэнд, ценовой триггер, λ, стиль исполнения, блэкаут. Открытая позиция до конца работает по параметрам, замороженным при её открытии.
- **Касается только будущей структуры:** экспирация, офсет крыльев, количество контрактов.
- **«Применить лучшие параметры»** после свипа переписывает только конструктор; открытая позиция и счёт не меняются.
- **Из интерфейса не меняются:** депозит $100, временной триггер (60 с), сброс счёта; остановить уже запущенный опрос тоже нельзя.

Следующий раздел «О модуле» показывает эти понятия на карточках интерфейса, а дальше каждый шаг разобран подробно: интерфейс и статусы данных (разделы 1–2), параметры (раздел 3), открытие (раздел 4), решения движка (раздел 5), аналитика и свип (разделы 6–7), журнал и закрытие (раздел 8), ограничения (раздел 9), практикум одной сделки (раздел 10).

---

## О модуле: стратегия и интерфейс
...
- **«ПРЕДУПР.»**: источник работает, но вернул ошибку по перпетуалу или ногам открытой структуры, либо не прошел гейт греков. Ошибки вспомогательных инструментов (дежурный набор для сигнала IV) статус не понижают и видны только в подсказке.
...
Блокирующие причины запрещают открытие. К ним относятся неверный размер, несовпадающие данные ног, нога без котировки и активный блэкаут. Equity представляет текущую стоимость бумажного счета: $100 плюс накопленный P&L. Предупреждение `IM > депозит` означает, что расчетная начальная маржа выше equity. Оно не блокирует paper trading, но такую позицию, вероятно, нельзя было бы открыть на реальном Standard Margin счете с теми же $100.
...
*Разложение итога: опционный результат (MTM), результат перпетуала, накопленный фандинг и комиссии формируют нетто-P&L.*
...
*Порядок цикла: сначала блэкаут, затем триггеры (Δ / ценовой / временной), округление до контрактов, фильтр издержек × `λ` и в конце вердикт **ХЕДЖ / ПРОПУСК / ПАУЗА**.*
...
Результаты сначала сортируются по соответствию маржинальному лимиту, затем по Sharpe, итоговому P&L и порядку сетки. Маржинальный лимит - текущий equity счета ($100 плюс накопленный P&L), тот же, с которым сверяется предупреждение тикета. Первая строка поэтому не обязательно имеет максимальный Sharpe среди комбинаций вне лимита.
...
Пустая таблица означает, что снимков еще нет. Если все комбинации исключены, таблица называет фактическую причину - чаще всего в первом снимке не было котировок выбранных ног, но это может быть и блэкаут или отсутствие опционов экспирации. Подождите новые снимки и повторите свип.
...
Для закрытия нажмите **«закрыть структуру»** два раза в течение 3.5 секунды. Движок моделирует закрытие перпетуала по рынку и фиксирует текущий опционный MTM. Закрытие выполняется только по свежим данным: при остановленном источнике, статусе **«УСТАРЕЛО»** или отсутствии цены перпетуала кнопка вернёт ошибку - включите источник и дождитесь **«LIVE»**. Накопленный P&L не обнуляется. Источник продолжает получать данные для сигнала IV.
...
![Карточка «P&L счёта · атрибуция»](images/practicum/step5-pnl-attribution.png)
*Обе стороны баланса из шага 0 в живых числах: приток (опционный MTM, перпетуал, фандинг) против утечек (комиссии и распад опционов). В этом прогоне: опционы `+$0.02`, перп `−$0.01`, нетто `≈$0`, equity `$100`, поддерживающая маржа `$89`.*
...
![Карточка «Рыночные данные Deribit» в статусе LIVE](images/practicum/step1-market-data.png)
*Так выглядит готовность к работе: статус **LIVE**, `underlying_price` и `index_price` заполнены, **гейт греков OK · данные свежие**, а под карточкой - отметка времени UTC и «0с назад». Именно на эту карточку смотрят перед каждым решением.*
...
![Карточка «IV-режим · сигнал входа» сразу после запуска](images/practicum/step2a-iv-early.png)
*Сразу после запуска источника: **«окно 24ч · 0 набл.»**, ATM IV и ранг ещё прочерки, внизу - **«мало данных - режим ещё копится»**. Это нормальное начальное состояние, а не поломка: вердикту нужно накопить наблюдения.*

![Карточка «IV-режим · сигнал входа» с вердиктом](images/practicum/step2b-iv-verdict.png)
*Через ~6–7 минут (здесь - 15 наблюдений) появляется вердикт. В этом прогоне: ATM IV `34.1%`, **IV-ранг `0.00` - «у низа окна»**, вердикт **«вход благоприятен · IV ниже режима»** (ранг ≤ 0.35). DVOL `38.0%` показан для контекста и на вердикт не влияет.*
...
![Карточка «Маржа и риск»](images/practicum/step3-margin.png)
*Живой пример на размере 0.01: **IM `$119.07`**, поддерживающая **MM `$89.30`**, утилизация MM **`89%`** (красная полоса), equity **`$99.92`**. Внизу - метка **«маржа $119 превышает депозит $100 - структура вне лимита»**. Начальная маржа считается по двум проданным крыльям и имеет пол; сравните её с депозитом $100.*
...
1. **Экспирацию.** Авто-выбор виден в строке экспирации тикета как пометка «авто» перед выбранной датой (например, 12JUL); ручная правка экспирации в конструкторе эту пометку снимает.
...
![Тикет подтверждения: страйки, дебет, готовность данных, причины проверки](images/practicum/step4-ticket.png)
*Тот самый тикет из шага 4. Здесь: **экспирация выбрана автоматически (14JUL)**, страйки `62,000 · 68,000 · 57,000`, **нетто-дебет `$6.37`** (он же макс. убыток), параметры движка заморожены в строке. Строка **«есть предупреждения - открытие возможно»** и метка **⚠ предупр. IM $119 > депозит $100** - это ровно предупреждение о марже из шага 3: подтверждению оно не мешает (`✕ блок` запретил бы, `⚠ предупр.` - нет).*
...
![Карточка «Хедж-движок · решение» с вердиктом ПРОПУСК](images/practicum/step5-decision.png)
*Типичный цикл на размере 0.01: все три триггера (Δ · ценовой · временной) **«в норме»**, поэтому вердикт - **ПРОПУСК** («ордер не требуется · edge сохранён»). Это не поломка: движок экономит издержки, пока хедж не нужен.*

![График «Дельта и дедбэнд»](images/practicum/step5-delta-chart.png)
*Так выглядит работающий дельта-хедж: линия `Total Δ` возвращается в коридор дедбэнда (закрашенная полоса ±0.001) после каждой отметки исполнения. Синие метки - бумажные исполнения хеджа.*
...
![Полоса зоны Ⅱ с кнопкой закрытия в состоянии подтверждения](images/practicum/step6-close-armed.png)
*Закрытие - двойное нажатие в полосе зоны Ⅱ в течение 3.5 секунды. После первого нажатия кнопка меняется на **«✕ подтвердить закрытие?»** (как на снимке) и ждёт второго; если не нажать - сама вернётся в исходное состояние.*
...
![Таблица «Журнал хеджей и начислений» после закрытия](images/practicum/after-journal.png)
*Полный жизненный цикл одной сделки в журнале: открытие, затем серия хеджей, затем закрытие перпетуала и фиксация опционов. Статус **«✓ сходится»** означает, что компоненты P&L арифметически сходятся к итогу. Разберите: сколько было хеджей и сколько ушло на комиссии (экспорт всегда полный - CSV / XLSX / JSON).*

![Карточка «Метрики прогона»](images/practicum/after-metrics.png)
*Замороженная сводка завершённого прогона: Sharpe по тикам реприса (несравним с дневным/годовым), доля прибыльных циклов, просадка, число и средний размер хеджей, макс. отклонение дельты. Смотрите на процесс, а не на одну цифру P&L.*

![Таблица «Свип параметров»](images/practicum/after-sweep.png)
*Свип прогнал 108 комбинаций через тот же движок на одних снимках (здесь - 92 снапшота) и отсортировал по цели. Строка **«лучший»** - верхняя; но у всех комбинаций метка **«IM > деп.»**, поэтому **«применить лучшие параметры»** недоступно. И главное ограничение: это прошлое, не прогноз.*
