...
//
// КНИГ ШЕСТЬ, И ПЕРВЫЕ ДВЕ ПРО ДРУГОГО БОТА. Две книги схемы продавца (эталон и живой движок бота 2)
// снимаются с пятилетней записи, четыре последние - с годовых фикстур репозитория и описывают
// бота 1. Своей сквозной сверки у бота 1 не было вовсе: его охрана состояла из аналитических
// golden-тестов, а слой, который и есть «результат бота», под охраной не стоял. Они дешёвые
// (секунды, книга выхода около 30 с, записи не требуют), поэтому идут последними.
//
// ПОЧЕМУ У БОТА 1 ЧЕТЫРЕ КНИГИ, А НЕ ОДНА. Они стерегут ЧЕТЫРЕ РАЗНЫХ СЛОЯ, и подменять один другим
// нельзя. `base-fa.tsv` снимается БЕЗ баз фандинга: это поведение леджера до правила разбавления, и
// книга стережёт, что правка ничего не сдвинула там, где сдвигать не собиралась. `base-fa-dil.tsv`
// снимается С базами: тот же леджер эмулирует РЕАЛЬНЫЙ вход, умножая котируемую ставку на B/(B+S).
// `base-fa-size.tsv` это уже не леджер, а ПРАВИЛО ВХОДА: какой размер ставить на каждый рынок и
// какие рынки финансировать вовсе. Между «каждое ограничение верно» и «решение верно» помещается
// целый класс дефектов - перепутанный порядок ограничений, потерянный потолок, отбор, посчитанный
// на размере, которым мы не войдём, - и юнит-тесты его не видят.
//
// `base-fa-exit.tsv` стережёт ПРАВИЛО ВЫХОДА, и его отличие от книги входа не в наборе чисел, а в
// роде решения. Вход это одиночный выбор, выход это ЦЕПОЧКА: держать, уйти в кэш или переложиться,
// и так раз за разом. Дефекты этого рода юнит-тест не видит по определению, потому что каждое
// отдельное решение в них корректно, а портится последовательность: холостой оборот, храповик по
// размеру, перепутанные брутто и нетто, потерянный отказ снабжения.
//
...
if (args.includes("--help")) {
  ...
  --drop-rule <имя>    КОНТРОЛЬ: заглушить одно правило и убедиться, что охрана это заметит.
                       ...
                       бот 1, разбавление: dilution-off | dilution-wrong
                       бот 1, правило входа: ceiling-off | room-off | tail-open | ticket-off |
                       ratio-off | flip-on | grid-coarse | bases-off | weighted-base-off |
                       impact-off | capital-off
                       бот 1, правило выхода: switch-off | sunk-in | same-market-skip |
                       horizon-half | cadence-720 (плюс общие ticket-off и ratio-off)`);
  process.exit(0);
}
...
// Контроли каждого бота живут своим набором: у леджера фандинг-арбитража нет ни полосы хеджа, ни
// гейта размера, и подсунуть ему чужое имя правила значило бы молча ничего не заглушить. Поэтому
// имена правил перечислены у КАЖДОЙ книги, а не проверяются вычитанием одного набора из другого.
const B2_DROPS = ["band-off", "size-off", "pick-off", "settle-late", "stop-off"];
const FA_DROPS = ["cost-off", "config-flip", "hl-off", "rows-half"];
const FA_DIL_DROPS = [...FA_DROPS, "dilution-off", "dilution-wrong"];
// Контроли ПРАВИЛА ВХОДА свои и с контролями леджера не пересекаются: там глушится начисление,
// здесь ограничения размера. Общее имя означало бы, что один контроль проверяет две книги и ни
// одну целиком.
const FA_SIZE_DROPS = ["ceiling-off", "room-off", "tail-open", "ticket-off", "ratio-off", "flip-on",
  "grid-coarse", "bases-off", "weighted-base-off", "impact-off", "capital-off"];
// Контроли ПРАВИЛА ВЫХОДА. Три из них (`switch-off`, `sunk-in`, `same-market-skip`) подменяют не
// настройку, а КРИТЕРИЙ, то есть проверяют чувствительность книги к конкретной ошибке конструкции.
// Имена `ticket-off` и `ratio-off` НАМЕРЕННО совпадают с именами книги входа: это одни и те же
// настройки правила входа, и обе книги обязаны замечать их подмену, каждая по-своему.
const FA_EXIT_DROPS = ["switch-off", "sunk-in", "same-market-skip", "horizon-half", "ratio-off",
  "ticket-off", "cadence-720"];
const ALL_DROPS = [...new Set([...B2_DROPS, ...FA_DIL_DROPS, ...FA_SIZE_DROPS, ...FA_EXIT_DROPS])];

const fail = (...lines) => { console.error(`\n${lines.join("\n")}`); process.exit(1); };

// Неизвестное имя правила раньше молча ничего не глушило, и охрана печатала ЗЕЛЁНЫЙ итог: опечатка
// в имени контроля превращала контроль в его отсутствие, причём с успокаивающим выводом.
if (DROP_RULE != null && !ALL_DROPS.includes(DROP_RULE)) {
  fail(`НЕИЗВЕСТНОЕ ПРАВИЛО КОНТРОЛЯ: «${DROP_RULE}»`,
    `Заглушить нечего, и охрана прошла бы ЗЕЛЁНОЙ, то есть контроль молча перестал бы быть контролем.`,
    `Известные имена: ${ALL_DROPS.join(" | ")}.`);
}
...
console.log(`  четыре книги бота 1 по годовым фикстурам: три по секунде, книга выхода около 30 с.`);
...
// Каждая книга объявляет, КАКИЕ контроли к ней относятся. Контроль, попавший не в ту книгу, ничего
// не заглушит и оставит охрану зелёной, а контроль без своей книги невозможно проверить на падение.
const BOOKS = [
  { file: "base-ref.tsv", title: `книга эталона`, script: "scripts/hist-sellhedge.mjs",
    heap: true, rec: true, drops: [], argv: ["--book-lots", "100"] },
  { file: "base-eng.tsv", title: `книга движка`, script: "scripts/replay-sellhedge.mjs",
    heap: true, rec: true, drops: B2_DROPS, argv: ["--qty", "1.0"] },
  // Книги бота 1 читают фикстуры репозитория, а не запись: ни `--dir`, ни расширенной кучи им не
  // надо. Контроли у них свои: правила у ботов разные, и глушить у леджера «полосу хеджа» нечего.
  { file: "base-fa.tsv", title: `книга бота 1`, script: "scripts/replay-funding.mjs",
    heap: false, rec: false, drops: FA_DROPS, argv: [] },
  // Базы фандинга лежат в репозитории рядом с записью (13 МБ сжатых снимков по 63 рынкам, из них
  // читаются три). Путь тот же, что в шапке test/baselines/books.sha256.
  { file: "base-fa-dil.tsv", title: `книга разбавления`, script: "scripts/replay-funding.mjs",
    heap: false, rec: false, drops: FA_DIL_DROPS, argv: ["--bases", "../data/funding-arb/gmx-oi-snapshots"] },
  // Книга ПРАВИЛА ВХОДА. Читает те же фикстуры плюс снимки ёмкости и измеренные кривые удара из
  // ../data/funding-arb; ни записи бота 2, ни сети не требует.
  { file: "base-fa-size.tsv", title: `книга правила входа`, script: "scripts/replay-funding-size.mjs",
    heap: false, rec: false, drops: FA_SIZE_DROPS, argv: [] },
  // Книга ПРАВИЛА ВЫХОДА. Те же фикстуры и те же снимки, что у книги входа, но обход по цепочке
  // решений с кадансом 24 ч, поэтому она дороже прочих книг бота 1: около 30 с против секунд.
  { file: "base-fa-exit.tsv", title: `книга правила выхода`, script: "scripts/replay-funding-exit.mjs",
    heap: false, rec: false, drops: FA_EXIT_DROPS, argv: [] },
];
...
for (let i = 0; i < BOOKS.length; i += 1) {
  ...
  const r = run(process.execPath, [
    ...(b.heap ? NODE_BIG : []), b.script, ...(b.rec ? ["--dir", REC] : []), ...b.argv,
    ...(DROP_RULE && b.drops.includes(DROP_RULE) ? ["--drop-rule", DROP_RULE] : []),
    "--book", path,
  ]);
...
if (!digest.ok) {
  ...
  if (bad.has("base-fa-size.tsv")) {
    advice.push(``);
    advice.push(`Правило ВХОДА (base-fa-size.tsv): книга поблочная, столбцы идут в порядке решения.`);
    advice.push(`Первый разошедшийся столбец называет сдвинувшийся шаг: «база взвеш.» - статистика`);
    advice.push(`базы для О2, «потолок» и «связывает» - ограничения, «оптимум» - поиск по сетке,`);
    advice.push(`«размер» - потолок тикета и сжатие, «отношение» - отбор рынка, «выдано» -`);
    advice.push(`распределитель капитала. Сдвинулся ТОЛЬКО столбец «отказ» - изменился порядок`);
    advice.push(`проверок, а не арифметика.`);
  }
  if (bad.has("base-fa-exit.tsv")) {
    advice.push(``);
    advice.push(`Правило ВЫХОДА (base-fa-exit.tsv): книга это ЦЕПОЧКА решений, поэтому адрес ищется`);
    advice.push(`ПЕРВОЙ разошедшейся строкой, а не последней: одно сдвинувшееся решение уводит весь`);
    advice.push(`остаток книги, и последние строки расходятся как следствие, а не как причина.`);
    advice.push(`Столбцы идут в порядке решения: «брутто удержания» - ветка удержания, «нетто альт»`);
    advice.push(`- вызов правила входа, «прибавка» - сам критерий, «действие» и «причина» - выбор`);
    advice.push(`ветки. Сдвинулось ТОЛЬКО «нетто альт» - причина в правиле входа, и книга`);
    advice.push(`base-fa-size.tsv обязана была разойтись тоже; разошлась одна эта - причина в выходе.`);
  }
  if (bad.has("base-fa.tsv") || bad.has("base-fa-dil.tsv")) {
    advice.push(``);
  ...
  if (bad.has("base-fa-dil.tsv") && !bad.has("base-fa.tsv")) {
    advice.push(``);
    advice.push(`Разошлась ТОЛЬКО книга разбавления - сдвинулось правило B/(B+S), а не начисление.`);
    advice.push(`Смотреть столбцы «множитель» и «без базы»: первый двигают правила применения`);
    advice.push(`(fa/dilution.js), второй означает, что пропали базы фандинга, а не изменилось правило.`);
  }
  advice.push(``);
