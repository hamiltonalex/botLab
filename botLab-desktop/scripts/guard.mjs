...
//
// КНИГ ЧЕТЫРЕ, И ПОСЛЕДНИЕ ДВЕ ПРО ДРУГОГО БОТА. Две книги схемы продавца (эталон и живой движок
// бота 2) снимаются с пятилетней записи, две последние - с годовых фикстур репозитория и описывают
// бумажный леджер БОТА 1. Своей сквозной сверки у бота 1 не было вовсе: его охрана состояла из
// аналитических golden-тестов, а слой, который и есть «результат бота», под охраной не стоял.
// Они дешёвые (доли секунды, записи не требуют), поэтому идут последними и ничего не удорожают.
//
// ПОЧЕМУ У БОТА 1 ДВЕ КНИГИ, А НЕ ОДНА. Один и тот же леджер умеет считать двумя способами, и оба
// обязаны стоять под охраной. `base-fa.tsv` снимается БЕЗ баз фандинга: это поведение до правила
// разбавления, и книга стережёт, что правка ничего не сдвинула там, где сдвигать не собиралась.
// `base-fa-dil.tsv` снимается С базами: тот же леджер эмулирует РЕАЛЬНЫЙ вход, умножая котируемую
// ставку на B/(B+S). Возможность, которую охрана ни разу не исполняет, сгнивает незаметно, а тут
// речь о правиле, из-за отсутствия которого счёт год показывал несуществующую прибыль.
//
...
const ALL_DROPS = [...new Set([...B2_DROPS, ...FA_DIL_DROPS])];
