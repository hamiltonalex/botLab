...
//
// ПОЧЕМУ ПРОПАВШИЙ ИНСТРУМЕНТ НЕЛЬЗЯ ПРОСТО ПРОПУСТИТЬ, и почему здесь это вреднее, чем в других
// скриптах. Раньше строка `if (!r1 || !fin(r1.m) || !fin(r1.b)) continue;` выбрасывала наблюдение,
// когда инструмент переставал восстанавливаться (спот ушёл от страйка, правило П3 в hist-surface.js
// не продолжает смайл за наблюдённые страйки). Вреда два, и второй специфичен для этого раздела:
//   1. ОТБОР ПО ИСХОДУ. Пропадают ушедшие далеко, а у покупателя далеко это либо крупная прибыль,
//      либо ноль. Замер на годовой записи: выбрасывалось 48 / 107 / 408 наблюдений на 8 / 12 / 24 ч,
//      и подавляющее большинство из них были В ДЕНЬГАХ на горизонте.
//   2. РЯД ПЕРЕСТАЁТ БЫТЬ РАВНОМЕРНЫМ ПО ВРЕМЕНИ. `acf` считает корреляцию по ИНДЕКСНЫМ лагам, а
//      лаг здесь переводится в часы через шаг записи (`Math.round(h·60/stepMin)`). Выброшенный
//      элемент сдвигает все последующие, то есть портит ровно ту величину, ради которой раздел и
//      написан. Это не поправка к n_eff, это условие её осмысленности.
// Поэтому наблюдение ОСТАЁТСЯ НА СВОЁМ МЕСТЕ, а цена продажи берётся как ВНУТРЕННЯЯ стоимость по
// форварду: строгая нижняя граница цены опциона, то есть оценка, которая покупателю НЕ ЛЬСТИТ
// (спред при этом не платится, что играет в другую сторону, но он меньше отброшенной временной
// стоимости). На ЖИВОЙ записи ничего этого не происходит и счётчик равен нулю.
function series(H) {
  const out = [];
  const st = { synth: 0, noCand: 0, expired: 0 };
  for (let i = 0; i < times.length; i++) {
    const t = ticks.length ? tickAt(times[i]) : null;
    const side = t?.sd === "put" ? "P" : "C";
    const spot = fwd[i];
    if (!fin(spot)) { st.noCand += 1; continue; }
    let best = null;
    for (const r of snaps.get(times[i]).values()) {
    ...
    if (!best) { st.noCand += 1; continue; }
    const r0 = best.r;
    const tgt = times[i] + H * 3600000;
    let j = i + 1;
    while (j < times.length && times[j] < tgt) j++;
    if (j >= times.length) break;
    // Экспирация раньше горизонта окном отбора исключена (в кандидаты берётся только r.h ≥ 48 ч,
    // а горизонты здесь 8-24 ч), поэтому терминальная выплата тут не считается. Если окно или
    // горизонты когда-нибудь разъедутся, такое наблюдение будет ПОСЧИТАНО и напечатано, а не
    // молча оценено неверной формулой.
    if (fin(r0.e) && r0.e <= tgt) { st.expired += 1; continue; }
    const r1 = snaps.get(times[j])?.get(r0.n);
    const paid = r0.a * 0.01 + feeUsd(r0.m, r0.f) * 0.01;
    let got;
    if (r1 && fin(r1.m) && fin(r1.b)) got = r1.b * 0.01 - feeUsd(r1.m, r1.f) * 0.01;
    else {
      const fj = fwd[j];
      if (!fin(fj)) { st.noCand += 1; continue; }
      const intr = Math.max(0, r0.s === "C" ? fj - r0.k : r0.k - fj);
      got = intr * 0.01 - feeUsd(intr, fj) * 0.01;
      st.synth += 1;
    }
    out.push(((got - paid) / paid) * 100);
  }
  out.stats = st;
  return out;
}
...
console.log(`| удержание | наблюдений | из них по внутр. стоимости | корр. на 0.5 ч | 2 ч | 6 ч | 12 ч | **n_eff** | часов на одно независимое |`);
console.log(`|---|---|---|---|---|---|---|---|---|`);
...
for (const H of [8, 12, 24]) {
  ...
  effs.push({ H, nEff });
  noCandMax = Math.max(noCandMax, xs.stats.noCand);
  console.log(`| ${H} ч | ${xs.length} | ${xs.stats.synth}${xs.stats.expired ? ` (+${xs.stats.expired} с экспирацией до горизонта, исключены)` : ""} | ${at(0.5)} | ${at(2)} | ${at(6)} | ${at(12)} | **${f(nEff, 1)}** | ${f(spanH / nEff, 1)} ч |`);
}
if (noCandMax) {
  console.log(`\nСнимков без подходящего кандидата (нет опциона нужной стороны с дельтой около 0.45 в окне`);
  console.log(`48-336 ч): ${noCandMax}. Они РАЗРЕЖИВАЮТ ряд, и лаги в часах выше поэтому приблизительны:`);
  console.log(`\`acf\` считает по индексным лагам, а подпись переводит их через шаг записи. Это свойство`);
  console.log(`снабжения, а не исхода, поэтому отбором по результату оно не является, но знать о нём надо.`);
}
