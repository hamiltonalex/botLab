...
for (let i = 0; i < N; i++) {
  ...
  if (open && !st.structure) {
    ...
    open = null;
    // КОНТРОЛЬ: сдвиг стоит ПОСЛЕ книжения закрытой сделки, поэтому на сутки уезжает ВХОД
    // СЛЕДУЮЩЕЙ сделки, а не выход текущей; сверка обязана заметить со сделки 2.
    if (DROP === "settle-late") i += 24;
  }
  void cycle;
}
