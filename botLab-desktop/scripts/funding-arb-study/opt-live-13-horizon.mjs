...
const H = Number(process.argv[2] || HOURS_PER_YEAR);
const rows = [];
...
// разложение оптимума: сколько даёт разбавляемый котёл GMX, а сколько неразбавляемая нога HL
let gmxPart = 0, hlPart = 0, borrowPart = 0;
for (const r of rows) {
  if (r.skip || !r.best || r.best.net <= 0) continue;
  const b = r.best;
  const m = byAddr.get(A.mkt[r.t].market.toLowerCase());
  const can = gmxMarketToCanonical(m);
  const i = hlIdx.get(r.coin);
  const hl = hlCtxToCanonical(r.coin, meta.universe[i], ctxs[i]);
  const a0 = annualizeRow({ f_long: can.factors.f_long, f_short: can.factors.f_short, b_long: can.factors.b_long, b_short: can.factors.b_short, hl_rate: hl.hl_rate });
  const isA = b.cfg === "A";
  gmxPart += b.s * (isA ? a0.gmx_short_recv : a0.gmx_long_recv) * b.dil * (H / HOURS_PER_YEAR);
  borrowPart += b.s * (isA ? a0.gmx_borrow_short : a0.gmx_borrow_long) * (H / HOURS_PER_YEAR);
  hlPart += b.s * (isA ? a0.hl_long_recv : a0.hl_short_recv) * (H / HOURS_PER_YEAR);
}
console.log(`горизонт H=${H} ч: вклад разбавленного котла GMX $${gmxPart.toFixed(0)}, займа GMX -$${borrowPart.toFixed(0)}, ноги HL $${hlPart.toFixed(0)}`);
