// adv-15-y2cap.mjs - ВТОРОЙ ПЕРИОД С ПОТОЛКОМ ТИКЕТА, ПРИВЕДЁННЫМ К КАПИТАЛУ (2026-09-04). READ-ONLY.
//
// ЗАЧЕМ. Запись проекта «вне выборки правило проиграло hold вдвое: $214.80 против $429.99» получена
// стендом adv-6-y2.mjs при капитале $2500 и потолке тикета правила $5000 (FA_SIZING_DEFAULTS). В этой
// раскладке рынок назначения выбирается по нетто при ЕГО оптимуме до $5000 (candidates сортирует по
// c.n), а капитал $2500 потом раскладывается по оболочке того же рынка: место назначения выбрано при
// одном размере, а исполняется при другом. Живой автомат так не делает: auto.js приводит потолок
// тикета к капиталу сделки, и рынок выбирается по нетто при размере, которым бот войдёт.
//
// ЧТО ЗДЕСЬ ИЗМЕНЕНО ПРОТИВ adv-6-y2.mjs: РОВНО ОДНА СТРОКА, потолок тикета в срезе равен
// min($5000, капитал). Ходок pf-walk.mjs, данные, каданс, старты и длина те же.
//
// ИТОГ ЗАМЕРА (окно 2024-11-30..2025-06-20, 22 рынка, капитал $2500, 10 стартов, медиана в год):
//   потолок $5000 (adv-6 как есть, воспроизведено 04.09):  правило $219.21, hold $389.98;
//   потолок $2500 (этот файл):                             правило $712.31, hold $412.83.
// То есть на втором периоде ПРАВИЛО БЬЁТ БАЗУ, а записанный проигрыш вдвое был свойством стенда, а
// не правила. Оговорка прежняя: во втором периоде только крупные имена, малых (BERA, ANIME, PEPE,
// SHIB, APT), на которых заработан год 1, там нет; воспроизводимость по вселенной этим не доказана.
// Независимое подтверждение другим ходоком: exit-6-loss.mjs --period y2 (правило $578, hold $428 в год
// по 12 стартам с шагом 60 ч).
//
// Запуск: node adv-15-y2cap.mjs 1732924800 y2cap   (аргумент это начало окна в unix-секундах)
...
const CAPITAL = 2500;
// ЕДИНСТВЕННОЕ ИЗМЕНЕНИЕ ПРОТИВ adv-6-y2.mjs: потолок тикета приведён к капиталу, как у автомата.
const CFG_CAP = { ...FA_SIZING_DEFAULTS, ticketCapUsd: Math.min(FA_SIZING_DEFAULTS.ticketCapUsd, CAPITAL) };
...
for (let t = H; t <= n; t += 24) {
  ...
  const u = sizeUniverse({ markets: slice, costs: DEFAULT_COSTS, capitalTotal: 1e9, cfg: CFG_CAP });
  const ok = [];
...
for (const mode of ["rule-1", "hold-1"]) {
  const nets = [], opens = [], toks = [];
  for (const f of STARTS) {
    const r = walk({ scan, env, capital: CAPITAL, cadence: 24, kmax: 1, mode, first: f, last: f + LEN });
    nets.push(r.net * YM); opens.push(r.tally.open);
    const s0 = r.log.find((l) => l.act === "set"); toks.push(s0 ? s0.tokens : "-");
  }
  console.log(mode, "длина", LEN, "ч =", (LEN / 24).toFixed(0), "сут; нетто/год медиана", $(q(nets, 0.5)),
    "мин", $(Math.min(...nets)), "макс", $(Math.max(...nets)), "в плюсе", nets.filter((x) => x > 0).length + "/" + nets.length,
    "кругов", q(opens, 0.5), "первый рынок", [...new Set(toks)].join(","));
}
