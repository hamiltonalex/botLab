...
const XS=HL.meta.xs, CAP_SEC=1e-7, E30=1e30, MIN_SEC=3.1710e-10; // 1% годовых, минимум протокола
...
// mode: "raw" ставка как есть | "dil" разбавление | "flipLo" +смена знака по минимуму | "flipHi" +смена знака по ставке плательщика
const STAT={};
function transform(rows,cfg,S,mode,key){ if(mode==="raw") return rows; const isShort=cfg==="A";
  return rows.map(r=>{ const f=isShort?r.f_short:r.f_long; if(!(f>0)) return r;   // мы платим: ничего не трогаем
    const ours=isShort?r.bs:r.bl, opp=isShort?r.bl:r.bs;
    if(!(ours>0&&opp>0)) return r;
    if(key){STAT[key]=STAT[key]||{h:0,flip:0};STAT[key].h++;}
    if(mode!=="dil" && ours+S>opp){ if(key)STAT[key].flip++;
      const pay = mode==="flipLo" ? MIN_SEC : Math.abs(isShort?r.f_long:r.f_short);
...
function run({SRC,capital,N=3,K=8,share=0.01,margin=5,mode,key}){
  ...
  for(let i=trainH;i+24<=YEAR;i+=holdH){const te=Math.min(YEAR,i+holdH);if(te-i<24)break;
    ...
    for(const s of cand){if(now.size>=K||left<=1)break;
      ...
      w=transform(w,s.cfg,size,mode,key);
...
console.log("# 28 ЧИСТЫХ ИМЁН, ПОЛНАЯ СЕТКА. Строка тем честнее, чем правее столбец.");
console.log("# «-заморозки» = выброшены серии >=6ч побитово равных ставок. «доля потока» = N/(база+N) от потока плательщиков.");
console.log("# «смена знака» = если наша база + размер превысила базу противоположной стороны, мы платим (lo: по минимуму протокола 1%/год, hi: по ставке плательщика того часа).\n");
console.log("| капитал | доля HL | как в прогоне | -заморозки | -доля потока | -поток+знак lo | -поток+знак hi | -всё, знак lo | -всё, знак hi |");
console.log("|---|---|---|---|---|---|---|---|---|");
const best={};
for(const share of [0.005,0.01,0.02]) for(const capital of [100000,200000,300000,500000,1000000,3000000]){
  const f=x=>`$${Math.round(x.usd)}`;
  const a=run({SRC:BASE,capital,share,mode:"raw"});
  const b=run({SRC:D6,capital,share,mode:"raw"});
  const c=run({SRC:BASE,capital,share,mode:"dil",key:`${capital}|${share}`});
  const d=run({SRC:BASE,capital,share,mode:"flipLo"});
  const e=run({SRC:BASE,capital,share,mode:"flipHi"});
  const g=run({SRC:D6,capital,share,mode:"flipLo"});
  const h=run({SRC:D6,capital,share,mode:"flipHi"});
...
console.log("\n# ЛУЧШАЯ КЛЕТКА СЕТКИ ПО КАЖДОМУ СТОЛБЦУ (порог заказчика $25000-30000)");
for(const [n,v] of Object.entries(best)) console.log(`  ${n}: $${Math.round(v.usd)} при капитале $${(v.capital/1e6).toFixed(2)}M, доля HL ${(100*v.share).toFixed(1)}%`);
console.log("\n# ДОЛЯ ЧАСОВ-ПОЛУЧАТЕЛЯ, ГДЕ НАШ ВХОД ДЕЛАЕТ НАС БОЛЬШЕЙ СТОРОНОЙ");
for(const [k,v] of Object.entries(STAT)) console.log(`  ${k}: ${(100*v.flip/v.h).toFixed(2)}% (${v.flip} из ${v.h})`);
