...
// ЧТО ИМЕННО НЕВЕРНО:
//   1. Второй год НЕ ИЗМЕРЯЕТ РАЗНИЦУ РУК. При потолке $100k перевес равен $0.75 в год
//      ($1.11 суммарно за 18 блоков) при поблочном стандартном отклонении $384-$406:
//      перестановочный односторонний p=0.4994, бутстрап 48.0% отрицательных выборок,
//      95% ДИ [-$3552, $3082]. Сдвиг фазы календаря на 120-600 часов уводит фиксированную
//      руку в минус в 15 ячейках из 30, медиана по 30 ячейкам -$4 в год.
//   2. Числа $383/$110/$57 это нетто САМОЙ фиксированной руки, а не её перевес над
//      оппонентом. Читать их как «превосходство» нельзя.
//   3. Фиксированная рука на этом периоде выигрывает МОЛЧАНИЕМ: fixS попадает в нижний
//      узел сетки $10 в 12 блоках из 18, это ниже MIN_TICKET, и блок пропускается целиком.
//      54 сделки из 55 лежат в пяти последних блоках. Общий блочный затвор обеим рукам
//      переворачивает вердикт на двух потолках ($50k и $100k, оба за пер-рыночную).
//   4. «Половины периода расходятся по знаку» держится на ОДНОЙ сделке: единственная
//      сделка первой половины, блок 2023-12-24, $63096, нетто -$405.04.
//   5. ДЕФЕКТ УЧЁТА: один пропущенный час 2024-08-15T13:00 убивает блоки 2024-07-21 и
//      2024-08-20, реально считаны 18 блоков из 20, а шапка печатала «зачётных 20».
//   6. У 6 имён из 23 (AAVE, ATOM, AVAX, NEAR, OP, TAO) ставка тождественный ноль во ВСЕХ
//      часах периода. Проверяющий заключил, что это порча индексатора; ЭТО ЗАКЛЮЧЕНИЕ
//      ОПРОВЕРГНУТО (y2-4-zerorate.mjs): нули лежат в КЭШЕ ставок, а не в выгрузке баз, и
//      индексатор с кэшем СОГЛАСЕН (168 снимков из 168 с нулём против 168 из 168
//      ненулевых у BTC и LINK в том же окне). Фандинга на этих рынках тогда просто не
//      было. Чинить нечего, но их надо исключать как «нет предмета»: в отбор они не
//      проходят и так, а вот в выбор ЕДИНОГО размера входят своими отрицательными
//      вкладами, и это смещение надо снять явно.
//
...
// Блок, у которого ни один рынок не имеет полного покрытия И в нём, И в предыдущем, в счёт
// НЕ ИДЁТ. Печатать надо посчитанное, а не запланированное: один пропущенный час
// 2024-08-15T13:00 убивает два блока, и шапка «зачётных 20» скрывала, что считаны 18.
const REAL = TEST.filter((b) => MARKETS.some((m) => NET.get(m.t)?.has(b) && NET.get(m.t)?.has(b - H * 3600)));
console.log(`блоков по ${H} ч всего ${BLOCKS.length}, зачётных ${TEST.length}, РЕАЛЬНО посчитано ${REAL.length}` +
  `${REAL.length < TEST.length ? ` (${TEST.length - REAL.length} выпали из-за дыр в часах)` : ""}. Период с первым годом НЕ пересекается.\n`);
