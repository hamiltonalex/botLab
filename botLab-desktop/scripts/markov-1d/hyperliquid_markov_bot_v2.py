...
FEE_ROUND_TRIP = 0.001  # комиссия за вход и выход вместе: 0,1%, как в Binance-копии, чтобы обе считали одинаково (верхняя оценка); базовый тейкер Hyperliquid 0,045% на сторону, то есть 0.0009 за круг, можно поставить его
...
MAX_STALE_DAYS = 2  # если последняя закрытая свеча старше двух дней, свежих данных нет (монету сняли с биржи, как XMR, или сбой данных), сигнал не даём
...
def data_is_stale(rows):
    # Если свечи старые (монету сняли с биржи, как XMR в феврале 2024, или данные не пришли), раньше скрипт молча
    # давал сигнал по устаревшим данным. Теперь честно пишет, что свежих данных нет, и ничего не считает.
