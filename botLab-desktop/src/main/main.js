...
// FA_UNIVERSE_SCAN - собирать список рынков ЖИВЫМ ОТБОРОМ (`fa/universe-scan.js`) вместо зашитых
// пяти строк `universe.js`. С ВЕРСИИ 0.4.0 ВКЛЮЧЕНО ПО УМОЛЧАНИЮ; значение `0` возвращает поведение
// пяти имён побитово: список становится ровно `ALL_MARKETS`, ни одного лишнего запроса, ни одного
// лишнего байта записи. Решение владельца 18.09.2026: «нам нужно всегда использовать все новые
// монеты», и замер, на котором оно стоит, лежит в отчёте расширения вселенной от 16.09 (две монеты
// дают $40.55 в год, полусотня имён $559.11 при том же капитале и том же правиле).
//
...
// туда ВТОРОЙ флаг, `FA_UNIVERSE_APPLY`. Разделение сохранено: снабжение можно оставить включённым,
// а применение выключить, и тогда бот считает по пятёрке, но пишет состав полусотни.
//
// ОТКАЗ ОТБОРА НЕ ОСТАВЛЯЕТ БОТА БЕЗ РЫНКОВ: при недоступном снабжении список поднимается из
// сохранённого файла профиля, а при его отсутствии из тех же пяти зашитых имён (`источник fallback`
// в журнале буга).
const FA_UNIVERSE_SCAN = process.env.FA_UNIVERSE_SCAN !== "0";
// FA_UNIVERSE_APPLY - ПОДАТЬ живой список в срез правил (`faAutoMarkets`), то есть перестать
// считать по пяти именам. С ВЕРСИИ 0.4.0 ВКЛЮЧЕНО ПО УМОЛЧАНИЮ; значение `0` оставляет срез ровно
// на `ALL_MARKETS`, побитово как до фазы 3.
//
// ПРИМЕНЕНИЕ ТРЕБУЕТ СБОРА, и это не вкусовщина: без сбора список никто не греет кадрами и не
// доливает базами, и поданный в срез он состоял бы из рынков без истории, то есть полусотни отказов
// ворот покрытия вместо вселенной. Просьбу применить при выключенном сборе флаг НЕ исполняет молча,
// а называет в журнале: молчаливое «включил, но ничего не изменилось» это худший из возможных
// исходов для обкатки на удалённой машине.
//
// ЧТО ПРОИСХОДИТ С УЖЕ ОТКРЫТОЙ СДЕЛКОЙ ПРИ ОБНОВЛЕНИИ, названо честно. На первом же кадансе после
// перехода на 0.4.0 правило выхода сравнит её не с четырьмя альтернативами, а с полусотней, и может
// переложиться. Убытком это не является по устройству критерия: перекладка требует, чтобы нетто
// альтернативы перебило брутто удержания НА ПОЛНЫЙ КРУГ издержек, то есть происходит только там,
// где она сама себя окупает. Но МОМЕНТ такого решения назначает обновление, а не рынок, и поэтому
// он назван здесь и в CHANGELOG, а не оставлен сюрпризом. Кому важна ровно старая пятёрка под
// открытой сделкой, ставит `FA_UNIVERSE_APPLY=0` до её закрытия.
const FA_UNIVERSE_APPLY = process.env.FA_UNIVERSE_APPLY !== "0" && FA_UNIVERSE_SCAN;
if (process.env.FA_UNIVERSE_APPLY !== "0" && !FA_UNIVERSE_SCAN) {
  console.warn("[fa-univ] отбор вселенной выключен (FA_UNIVERSE_SCAN=0) - применение ВЫКЛЮЧЕНО тоже, срез остаётся на пяти именах");
}
...
function faAutoMarkets() {
  ...
  return buildFaSlice({
    ...
    nowMs: Date.now(),
    gmxAt: f.gmxAt,
    // ОКНО ОЦЕНКИ, ПО КОТОРОМУ ВЫБИРАЕТСЯ СТОРОНА НОГИ. То же число, по которому тик режет кадр и
    // считает ворота (`autoWindowH` от замороженных параметров), а не второе рядом: сторона,
    // выбранная на одном окне, и брутто, посчитанное на другом, описывали бы разные рынки.
    windowH: faWindowH(),
...
async function resolveBtcOptSellStructureLive(params) {
  ...
  // ПРАВИЛО ВЫХОДА ВООРУЖАЕТСЯ ЗДЕСЬ ЖЕ И ТОЖЕ НЕ ЯВЛЯЕТСЯ НАСТРОЙКОЙ. Уровень считает код от
  // потолка правила размера (`liveSellStop`), потому что оба числа описывают один аппетит к риску
  // и порознь разъедутся. Тумблера у оператора нет намеренно: величина выведена замером, а не
  // вкусом. Перекрытие остаётся возможным для прогонов и экспериментов, но приходит параметром
...
app.whenReady().then(async () => {
  ...
  // S1_AUTOSTART=1 это тот же приём для бота 2, и нужен он по той же причине, но острее. Опрос бота 2
  ...
  // А6 (fault-tolerance, находка C2): сон/пробуждение видимы и обрабатываются. Пока машина
  // действительно спит, setInterval не тикает - этих тиков не существует. На пробуждении
  // работающие источники рефрешатся сразу, не дожидаясь своего интервала: детекторы свежести и так
  // деградируют честно (ages огромные, unknown), но немедленный тик возвращает LIVE быстрее и
  // ставит точную метку возобновления в телеметрию. Догоняющего шквала нет по построению
  // (inFlight-гвард в createRestSource + один слот setInterval после сна).
  // fa-поллер сознательно НЕ трогаем: его гэп-механика - вопрос R2 ратификации (см. план А6).
  //
  // ВАЖНО, установлено живой пробой А6 2026-07-20 (§3 п.3): «сон» НЕ монолитен. macOS чередует
  // Maintenance Sleep и DarkWake - машину будят приходящие пакеты при активном TCPKeepAlive, и в
  // тёмных пробуждениях наши таймеры тикают, а биржа отвечает. Событие resume при этом приходит
  // РОВНО ОДИН раз, на полном пробуждении. В пробе окно между suspend и resume составило 31.9 мин,
  // но настоящего сна в нём было 19.1 мин, а 26 тиков прошли живьём (одно окно DarkWake длилось
  // 9 минут с идеальным кадансом 30с). Поэтому строки ниже НЕ обещают отсутствия тиков и называют
  // величину тем, что она есть, - окном с момента объявления о засыпании, а не длительностью сна:
  // иначе оператор вычтет из покрытия обкатки втрое больше, чем потеряно на самом деле.
  let suspendedAt = null;
  powerMonitor.on("suspend", () => {
    suspendedAt = Date.now();
    console.log("[main] система засыпает - таймеры встают; часть окна машина может работать в тёмных пробуждениях, тогда тики будут");
  });
  powerMonitor.on("resume", () => {
    ...
    console.log(
      `[main] полное пробуждение${gapMin != null ? `, окно с объявления о засыпании ~${gapMin} мин (часть его машина могла работать в тёмных пробуждениях)` : ""}` +
        " - немедленный рефреш работающих источников",
    );
    try {
