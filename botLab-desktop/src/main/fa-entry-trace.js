...
export function advanceFaEntryTrace(trace, event) {
  ...
  return applyFaEntryTraceEvent(clone(trace), event);
}

// ОТДЕЛЬНЫЙ СНИМОК ТРАССЫ. Зовётся тем, кто СОБИРАЕТСЯ отдать трассу наружу, а не тем, кто её
// копит: копящему отсоединение не нужно, потому что он владелец, а отдающему нужно всегда.
export const snapshotFaEntryTrace = (trace) => (trace ? clone(trace) : null);

// НАКОПЛЕНИЕ НА МЕСТЕ, БЕЗ КОПИИ. То же событие и тот же результат, что у `advanceFaEntryTrace`,
// но трасса меняется, а не пересоздаётся.
//
// ЗАЧЕМ ВТОРАЯ ДВЕРЬ В ОДНО ЗДАНИЕ, И ЭТО ЗАМЕР, А НЕ ВКУС. `advanceFaEntryTrace` копирует ВСЮ
// трассу на КАЖДОМ проверенном размере, и до расширения вселенной это стоило копейки: пять рынков
// это 10 кандидатов, 299 событий и снимок 23 КБ. ЗАМЕР 16.09 (стенд `trace-cost.mjs`, капитал
// $2500) на полусотне инструментов: 102 кандидата, 2537 событий, снимок до 203 КБ, и решение
// дорожает с 530 мс до 3939 мс - то есть 3.4 секунды из 3.9 это копирование, и главный процесс
// стоит всё это время синхронно: поллер, IPC и интерфейс ждут. На 63 рынках 6481 мс, и потолка у
// роста нет.
//
// ОТПРАВКА СНИМКА ДОРОЖЕ САМОЙ КОПИИ, И ЭТОГО НЕ СЧИТАЛИ ВОВСЕ. Каждое из N событий шлёт растущую
// структуру размера O(N), то есть трафик квадратичен: 288 МБ сериализации за ОДНО решение на 51
// рынке. При пяти именах это уже было 3.9 МБ, просто не стоило внимания.
//
// ПОЧЕМУ НЕ ПОТОЛОК КАНДИДАТОВ. Потолок отнял бы у карточки честности ровно то, ради чего она
// заведена: ответ «что бот видел по КАЖДОМУ рынку». Дорого не число кандидатов, а число КОПИЙ, и
// резать надо его.
//
// ЧТО ПРИ ЭТОМ НЕ ТЕРЯЕТСЯ. Образцы (`samples`) пишутся на каждом размере, как и раньше, и
// записанная трасса остаётся ПОБИТОВО той же: наружу уходит `snapshotFaEntryTrace`, а итог
// собирает `finishFaEntryTrace`, который отсоединяет сам. Живая анимация становится реже, и это
// ничего не стоит: при пяти именах перебор занимает 0.1 секунды и досмотреть его человек не
// успевал вовсе - ровно поэтому 07.09 и сделана кнопка повтора по записанным образцам.
export function applyFaEntryTraceEvent(trace, event) {
  if (event.type === "evaluation:start") return createFaEntryTrace(event);
  if (!trace) return null;
  const next = trace;
