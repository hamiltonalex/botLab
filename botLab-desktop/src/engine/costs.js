...
// РАЗБИВКА ПОД УЖЕ ПОСЧИТАННЫЙ КРУГ. Нужна там, где круг считает ПРАВИЛО, а не эта модель.
//
// ЗАЧЕМ. Правило входа считает круг по ИЗМЕРЕННЫМ кривым (удар GMX из снимка глубины и стакан
// Hyperliquid), и с 18.09 его число расходится с плоской моделью: `costAtSize` при живой кривой
// зануляет статью `gmxImpact` и подставляет измеренное. Леджер бумажной сделки списывал круг по
// модели, то есть по ДРУГОМУ числу, чем то, на котором принято решение. Замер на живой записи:
// правило посчитало $7.36, леджер списал $8.75, из них $2.50 статьёй удара, которую правило на том
// же рынке оценило в $0.00. Решение и учёт обязаны сходиться, иначе бот зарабатывает одним
// критерием, а отчитывается другим.
//
// КАК. Комиссии и газ здесь ТОЧНЫЕ, из модели: они и в правиле те же самые. Всё, что правило
// насчитало сверх них, это измеренное проскальзывание обеих площадок, и оно кладётся одной
// строкой. Тождество «сумма статей = круг» держится ПО ПОСТРОЕНИЮ, а не по совпадению, и это не
// украшение: интерфейс леджера показывает Σ детализации рядом с кругом целиком, и расхождение там
// видно глазом.
//
// «ПО ПОСТРОЕНИЮ» ЗДЕСЬ НЕ ЗНАЧИТ «ПОБИТОВО», и разница названа нарочно. Остаток считается
// вычитанием суммы четырёх статей, а интерфейс складывает пять в своём порядке, поэтому последний
// бит двоичной дроби может разойтись: на круге $7.36 расхождение 9e-16, то есть триллионные доли
// цента. Это ошибка округления, а не потерянные деньги, и тест держит допуск, а не равенство.
//
// ОТРИЦАТЕЛЬНОГО ОСТАТКА БЫТЬ НЕ МОЖЕТ, и если он вышел, разбивки НЕТ. Круг правила это те же
// комиссии плюс неотрицательные измеренные добавки, поэтому остаток ниже нуля означает, что модель
// издержек поменяли между решением и открытием. Тогда честнее вернуть `null` и списать круг целиком
// по модели, чем выдать статью с отрицательным проскальзыванием: смешивать два числа в одной
// строке хуже, чем взять одно из них.
export function breakdownForTotal(costs, notional, isOneLeg, totalUsd) {
  const b = roundTripCostBreakdown(costs, notional, isOneLeg);
  const total = Number(totalUsd);
  if (!b || !Number.isFinite(total)) return null;
  const fees = b.gmxOpenUsd + b.gmxCloseUsd + b.gmxGasUsd + b.hlTakerUsd;
  const slip = total - fees;
  if (!(slip >= 0)) return null;
  return { ...b, gmxImpactUsd: slip };
}
