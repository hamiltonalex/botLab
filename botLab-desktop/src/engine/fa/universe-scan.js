...
export const FA_UNIVERSE_DEFAULTS = Object.freeze({
  ...
  // Доля нашего размера в суммарном открытом интересе рынка, в процентах. Порог 10% ОСТАВЛЕН
  // решением владельца 17.09, и оставлен он не потому, что подобран, а потому, что любая его смена
  // это подбор константы по одному году.
  //
  // ЦЕНА ВОРОТ ПЕРЕМЕРЕНА, и прежние числа этой шапки ($724 без ворот, $455 при 25%, $526 при 10%,
  // «около 27% дохода») сняты как ОТВЕЧАВШИЕ НА ДРУГОЙ ВОПРОС. Они получены на списке имён,
  // проходящих ворота СЕГОДНЯ, замороженном и прогнанном по году, то есть меряли выживаемость в
  // сегодняшней ликвидности, а не ворота: из списка выпадали BERA, RENDER, PEPE, WIF, S и SHIB,
  // которые за тот год и принесли основной доход. Настоящий замер пересобирает состав на КАЖДОМ
  // решении по историческому интересу того часа (`gate5-rolling-oi.mjs`, год 8761 ч, 62 имени,
  // тикет $2500): без ворот $840.30, при 10% $729.04, то есть **цена ворот $111.26 в год, 13%**.
  //
  // ЧТО ВОРОТА ПОКУПАЮТ, ТОЖЕ ЗАМЕРЕНО, и это меньше, чем обещает название. Наша доля в рынке по
  // 8041 часу в позиции: ворота убирают 36% часов с долей выше 10% и 43% часов выше 25%, но ХВОСТ
  // НЕ ТРОГАЮТ ВОВСЕ (часов выше 100% при воротах 304 против 262 без них). Причина устройства:
  // ворота считают долю В МОМЕНТ ВХОДА, а позиция держится неделями, и схлопывание интереса под ней
  // не проверяет никто. У APT база фандинга падает до $12.37, у BERA до $1.52 при тикете $2500.
  //
  // ПОЭТОМУ ОБОСНОВАНИЕ ОСТАЁТСЯ НЕ В ДОХОДНОСТИ: ворота ограничивают величину, которую модель НЕ
  // умеет считать (второй порядок разбавления не измерен, и его верхняя оценка больше самого
  // разбавления, шапка `dilution.js`), и делают это лишь в середине распределения. 13% это премия
  // за единственную существующую защиту, а не за полную.
  //
  // ПОРОГ НЕ ПОДБИРАТЬ. Зависимость дохода от порога НЕ МОНОТОННА: 25% даёт $806.57, 5% даёт
  // $775.66, оба выше, чем 10%. Разброс по порогам шире разброса по стартам, то есть это не шум, но
  // и не порядок по строгости, и «порог X лучше порога Y» из этих чисел не следует.
  maxOiSharePct: 10,
...
export function selectUniverse({ marketsByChain = null, hlCoins = null, cfg = FA_UNIVERSE_DEFAULTS, asOfMs = null } = {}) {
  ...
  for (const r of rows) {
    ...
    //
    // ЗАМЕР ЦЕНЫ ЭТИХ ВОРОТ СДЕЛАН ПО ДРУГОЙ ВЕЛИЧИНЕ, и это надо знать, читая обоснование порога
    // (находка 7.5 аудита механики 18.09). Обоснование мерит базу ФАНДИНГА НАШЕЙ стороны («у APT
    // база падает до $12.37 под удерживаемой позицией»), а код мерит долю в сумме сторон. Величины
    // расходятся тем сильнее, чем перекошен рынок: на 551 655 часах ворота 10% проходят 59.3%
    // часов, и в 20.9% из НИХ тикет $2500 составляет больше 10% базы ПРИНИМАЮЩЕЙ стороны. То есть
    // ворота, настроенные по замеру базы нашей стороны, пропускают часы, в которых доля по этому
    // замеру уже превышена вчетверо чаще, чем кажется. Сама конвенция кода обоснована отдельно
    // (мёртвый рынок ловится суммой, а односторонняя база у мёртвого рынка бывает нулевой), и
    // менять её без замера нельзя: это ворота ОТБОРА, и они прямо касаются фазы 7.
