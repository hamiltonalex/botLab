...
//   * метка выбирает и ДОПУСК тождества в `resolveBase`: долитый час сверяется строго (1e-6, база и
//     ставки одного мгновения), живой час слабее (5%, `FA_IDENTITY_MAX_REL_ERR_LIVE`), потому что
//     живая база снята в первую минуту часа, а ставки строки индексатора относятся к его границе;
//     без этого треть живых часов BTC не проходила ворота (замер 2026-09-03 в шапке `dilution.js`).
...
// заполненный отсюда, помечается `fbase_src: "live"`: журнал пишется только живым опросом, и по этой
// метке `resolveBase` сверяет тождество сторон с допуском живой базы.
//
...
// ОКНО ДОЛИВА: часы ОКНА ОЦЕНКИ, у которых базы нет, среди ПРОШЛЫХ ПОЛНЫХ часов. Возвращает
...
// ещё не написана. Дальше окна оценки не тянется: 365 суток баз дёшевы, но решений не меняют.
//
// ИМЯ ПАРАМЕТРА ГОВОРИТ «ГОРИЗОНТ», А ПРИХОДИТ В НЕГО ОКНО (находка 7.9 аудита механики 18.09).
// Главный процесс передаёт сюда `faWindowH()`, то есть окно оценки назад, и это ВЕРНО по существу:
// доливать надо ровно те часы, по которым считаются ворота покрытия и брутто, а горизонт это
// масштаб вперёд. Сегодня оба числа равны 720, поэтому разницы не видно; фаза 7 планирует тронуть
// горизонт, и тогда окно долива разошлось бы с окном ворот без единого сообщения. Имя оставлено
// как есть (его читают два вызова и тест), но отныне названо: это ОКНО.
