...
// УЗЛЫ КРИВОЙ СТАКАНА. ЗАМЕР: проходка меряна ровно на этих восьми размерах, дальше измерений нет,
// и `allowExtrapolation: false` запрещает ходить за последний из них. Список заведён здесь, чтобы
// живой сбор и офлайн-прогон спрашивали стакан на ОДНИХ И ТЕХ ЖЕ размерах: разные наборы узлов дали
// бы разную интерполяцию издержек при одинаковых данных.
export const FA_BOOK_NODES_USD = Object.freeze([1000, 5000, 10000, 25000, 50000, 100000, 250000, 500000]);
...
//
//    ЭТО ПРО РАСПРЕДЕЛИТЕЛЬ, А НЕ ПРО АВТОМАТ (уточнено 2026-09-02). Автомат распределителем не
//    пользуется: он берёт лучшую кривую (`bestAlternative`), а её размер это оптимум золотого
//    сечения, обрезанный потолком тикета, приведённым к капиталу сделки. При упоре в потолок
//    работают ровно $2500, ниже потолка найденный оптимум, и узлы сетки тут ни при чём.
