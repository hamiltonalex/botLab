...
export const FA_SIZING_DEFAULTS = Object.freeze({
  ...
  //
  // ЭТО ОДНА КОНСТАНТА НА ВСЕ РЫНКИ, А НЕ ПОСЛЕДНИЙ ИЗМЕРЕННЫЙ УЗЕЛ КАЖДОГО (находка 7.4 аудита
  // механики 18.09). Ограничение НАЗЫВАЕТСЯ «за последний измеренный узел не ходить», а проверяется
  // против $500 000 у любого имени, тогда как у 21 рынка из 63 кривая стакана неполная, а у GMX
  // узлов всего четыре. На рабочем тикете цена этой подмены НОЛЬ: потолок $500 000 лежит выше
  // первого узла у всех рынков и связывать не может, пока капитал сделки $2500. Цена появится при
  // капитале примерно от $25 000, и тогда константу надо заменить на пер-рыночный край кривой, а не
  // поднимать.
...
// Взвешенная потоком база отвечает на вопрос «какая база в тех часах, откуда приходят деньги», и
// в обе стороны она права именно в этом смысле.
//
// ЧЕГО ЭТА БАЗА НЕ ОБЕЩАЕТ, и раньше здесь стояло обратное. О2 разрешает `S <= B_fw`, и при
// ПОСТОЯННОЙ базе такой размер давал бы удержание ровно 0.50. База не постоянна, поэтому сходства
// нет ни на одном имени. ЗАМЕР (`gate5-o2-vs-retention.mjs`, 63 имени, год 8761 ч, метод и
// множитель те же, что в `dilution.js`): фактическое удержание при разрешённом О2 размере выше
// 50% НИ У ОДНОГО имени, медиана 27.9%, ниже 25% у 28 имён из 63; даже у ETH 47.6%, у BTC 45.9%.
// Разрыв с настоящим порогом половинного удержания: медиана x7.6, максимум x22 376 (PEPE), у SEI
// разрешено $54.3 тыс при пороге $6.14, у CRV $211.1 тыс при $4.6 тыс.
//
// ПРИЧИНА СТРУКТУРНАЯ, а не в данных: множитель `B/(B+S)` ВОГНУТ по базе, поэтому среднее от
// множителя не превышает множителя от среднего ПРИ ЛЮБОМ распределении базы, и знак разрыва
// универсален. Величину разрыва задаёт РАЗБРОС базы по часам, а не её уровень: чем шире разброс, тем
// сильнее одно число завышает удержание. Никакая одночисловая сводка базы этого не чинит, в том
// числе и квантиль.
//
// ЧТО ИЗ ЭТОГО СЛЕДУЕТ. О2 это ПОТОЛОК РАЗМЕРА, а не обещание удержания: он не даёт стать рынком
// (без него `S*/B` доходит до 367%, см. `maxDilutionFraction`), но «половина» в его имени про
// базу, а не про доход. На расчёт дохода это не влияет вовсе: доход считается почасово, с почасовой
// базой, и хвост распределения учитывает верно.
//
// ЧАСЫ УПЛАТЫ В СЧЁТ НЕ ВХОДЯТ: разбавление их не трогает вовсе (правило 3), и множителя там нет.
//
// ЧЕМ ИМЕННО ВЗВЕШЕНО, СЛОВО В СЛОВО (находка 7.3 аудита механики 18.09). Вес это КОТИРУЕМАЯ ставка
// часа `f`, то есть считается `сумма(f*B)/сумма(f)`. Приходит нам не `f`, а `f * S * B/(B+S)`, то
// есть «поток получения» в названии функции это поток РЫНКА, а не наш доход. Направление подмены
// консервативное и это важно: нынешний вес переоценивает часы с МАЛОЙ базой (там ставка выше по
// модулю при том же потоке), поэтому взвешенная база выходит ниже, а потолок О2 жёстче
// правильного. Менять вес на наш доход значило бы ОСЛАБИТЬ ограничение, и делать это без замера
// нельзя.
...
// в разных местах.
//
// НАЗЫВАЕТСЯ ПЕРВЫЙ СРАБОТАВШИЙ КОД, А НЕ ВСЕ ПРИМЕНИМЫЕ (находка 7.6 аудита механики 18.09).
// Шапка `auto.js` («СБОР И РЕШЕНИЕ») требует обратного: «на тике вычисляются ВСЕ применимые коды...
// одноуровневая проверка показала бы оператору случайный из трёх». Здесь шесть последовательных
// `return`, то есть при одновременном сбое базы и стакана оператор увидит только базу. Разница не в
// решении (рынок отказан в любом случае), а во времени разбора при живом сбое снабжения: чинить
// придётся дважды. Собирать все коды значит менять форму ответа правила входа и запись решения,
// поэтому здесь это НАЗВАНО, а реестр старшинства автомата остаётся тем единственным местом, где
// сбор и решение разведены.
//
...
export function bestSizeForMarket({ token, config, strategy = "two", rows, live, costs = DEFAULT_COSTS, impact = null, cfg = FA_SIZING_DEFAULTS, uniformSizeUsd = null, onProgress = null }) {
  ...
  // ТРИ РАЗНЫЕ БАЗЫ ПОД ОДНИМ ОБОСНОВАНИЕМ, И ЭТО НАДО ЗНАТЬ (находка 7.2 аудита механики 18.09).
  // Потолок О2 считается здесь от базы, ВЗВЕШЕННОЙ ПОТОКОМ ОКНА (история, 720 часов назад);
  // `SPEC-bot1-sizing.md` 1.5 в псевдокоде пишет `sizeCeiling(live.bOwnUsd, ...)`, то есть базу
  // ЖИВОГО снимка; а замер цены потолка («снимает 14% головного числа») сделан, как SPEC сам
  // оговаривает, по МЕДИАННОЙ базе окна. Отношение взвешенной базы к медианной шапка
  // `flowWeightedBase` даёт по именам: BERA 0.04, FET 0.16, INJ 0.22, SOL 1.99, то есть на тесных
  // именах нынешний потолок В РАЗЫ жёстче измеренного, а на мажорах мягче. Содержательно живая база
  // ближе к предмету: рынком мы становимся в момент входа, против базы ЭТОГО часа, а не средней за
  // месяц. Переход на живую базу это правка ПРАВИЛА (она двигает размер каждого рынка и обе книги),
  // поэтому здесь только названо, а не сделано.
...
export const FA_SIZING_PRESETS = Object.freeze({
  ...
  //
  // КАЛИБРОВКА ЭТОГО ПРЕСЕТА В ЖИВОМ ТРАКТЕ НЕДОСТИЖИМА, и знать это обязательно (находка 7.7
  // аудита механики 18.09). Он задаёт `ticketCapUsd: Infinity`, а автомат опускает потолок тикета
  // до капитала сделки ДЛЯ ЛЮБОГО пресета (`auto.js`, приведение потолка к капиталу), потому что
  // иначе годной альтернативой считался бы размер, которым мы всё равно не войдём. Значит числа
  // строки `why` (единый размер $3162 на рынок при капитале $199 206) сняты на конфигурации,
  // которой у живого бота не бывает: у него потолок $2500. Пресет остаётся БАЗОЙ СРАВНЕНИЯ для
  // прогонов исследования, где потолок не приводится, и `calibrated: true` относится ровно к тем
  // прогонам, а не к бою.
  "fa-uniform-v1": Object.freeze({
    id: "fa-uniform-v1", calibrated: true,
    why: "оптимум единого размера $3162 на рынок, капитал $199206, $9507 в год (APR 4.77%), воспроизведён дважды; в живом тракте потолок тикета приводится к капиталу сделки, и эта калибровка там недостижима",
