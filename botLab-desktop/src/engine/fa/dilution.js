...
//
// ЧЕГО ЗДЕСЬ НЕТ, И ЭТО ГРАНИЦА, А НЕ ЗАБЫТАЯ ДЕТАЛЬ. Второй порядок эффекта НЕ измерен и в формулу
// не входит; верхняя оценка его величины больше самого разбавления. Поэтому `B/(B+S)` это ВЕРХНЯЯ
// оценка того, что нам достанется, и называть её «замером» нельзя: наблюдать поведение GMX с нашей
// позицией внутри рынка невозможно в принципе.
//
// У ЭТОГО ЭФФЕКТА ДВЕ СТОРОНЫ, И ДО 18.09 ЗДЕСЬ БЫЛА НАЗВАНА ТОЛЬКО ВЫГОДНАЯ (находка 7.11 аудита
// механики). Когда мы встаём на ПРИНИМАЮЩУЮ сторону, наш вход сокращает перекос и снижает ставку
// самого плательщика, то есть уменьшает наш доход, и `B/(B+S)` это верхняя оценка. Но в часы, когда
// платим МЫ, наш вход перекос УВЕЛИЧИВАЕТ, то есть поднимает ставку, которую платим мы: в живой
// сделке BTC/B таких часов было 124 из 191. Значит `B/(B+S)` завышает доход в одних часах и
// занижает расход в других, и обе стороны ненаблюдаемы по одной и той же причине. Правило 3
// (часы уплаты не масштабируются) от этого не становится неверным: оно запрещает УМЕНЬШАТЬ убыток,
// а не обещает, что убыток не вырос.
//
