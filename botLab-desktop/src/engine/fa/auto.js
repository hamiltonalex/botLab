...
export function defaultAutoParams() {
  return {
    ...
    decisionMaxAgeH: 72,
    // Требуемая доля часов окна с НАБЛЮДЁННОЙ базой фандинга. Единица это буквальное решение
    // владельца «пока не накопится 720 часов, правило входа отказывает», и она ДОПУЩЕНИЕ в той
    // части, что цена полного покрытия не измерена: часов без базы в данных 0.052% (288 из
    // 551 943), то есть на окне в 720 часов ожидается 0.38 дыры, и всякая дыра при значении 1
    // откладывает решение до её выхода из окна. Понижать это число можно только замером, и ровно
    // поэтому оно параметр, а не константа.
    //
