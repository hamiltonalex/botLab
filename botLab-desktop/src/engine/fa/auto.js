...
//
// ДОХОДНОСТЬ ПРАВИЛА ВНЕ ВЫБОРКИ ПРОВЕРЕНА НА ОДНОМ ПЕРИОДЕ, И ЭТОГО МАЛО. Второй период проекта
// (22 крупных имени, 2024-12..2025-06) при потолке тикета, равном капиталу, как здесь, даёт правилу
// $712 в год против $413 у простого «вошли и держим» (`adv-15-y2cap.mjs`, 2026-09-04); прежняя
// запись «$214.80 против $429.99, база бьёт правило вдвое» была свойством стенда с потолком $5000
// при капитале $2500. Малых имён, на которых заработан год 1, в том периоде нет, и воспроизводимость
// края по вселенной не доказана. Автомат
...
//
// ГРАНИЦА ЭТОГО ЗАКОНА, НАЗВАННАЯ ЧЕСТНО (находка 6.13 аудита механики 18.09). Закон держит ЗНАЧЕНИЯ
// полей, которые в замороженном наборе ЕСТЬ: их правка в умолчаниях открытую сделку не трогает. Он
// НЕ держит поля, которых при взводе не существовало вовсе: тик раскладывает набор как
// `{ ...defaultAutoParams(), ...st.params }`, и новое поле приезжает из сегодняшних умолчаний.
// Живой пример: замороженный набор от 01.09 содержит семь полей и не содержит ни
// `drawdownStopRounds`, ни трёх порогов событий, а сделка от 02.09 велась и событиями (выкатка
// 02.09), и стопом по просадке (выкатка 04.09 без перевзвода).
//
// ПОЧЕМУ ИМЕННО ТАК, А НЕ СТРОГО. Строгое чтение («нет поля значит нет поведения») отключило бы на
// открытой сделке сторож просадки, выкаченный после взвода, то есть выкатка защиты не защищала бы
// уже открытую сделку. Это решение, а не недосмотр, и цена его в долларах ноль, пока новое
// умолчание совпадает с желаемым. Чтобы отклонение не было молчаливым, тик возвращает список
// полей, приехавших из умолчаний (`paramsFromDefaults`): сделать заморозку строгой это решение
...
// ЧИСЛА, и в паспорте сделки пишутся оба. Здесь стояло «$2500 работают как $1995.26»: это про
// РАСПРЕДЕЛИТЕЛЬ, который ходит по узлам сетки (шапка `sizing.js`), а автомат берёт лучшую кривую
// по `bestAlternative` и потолок тикета приводит к капиталу сделки, поэтому при упоре в потолок
// работают ровно $2500 (тест «ВХОД: правило входа зовётся целиком»), а ниже потолка найденный
// оптимум. Проверено 2026-09-02, кандидат (3) сверки руководства закрыт.
//
...
export const FA_AUTO_REFUSALS = Object.freeze([
  ...
  // РЫНКА, НА КОТОРОМ МЫ СТОИМ, В СРЕЗЕ НЕТ ВОВСЕ. Не «истории мало» и не «баз мало»: рынок из
  // вселенной ВЫПАЛ, и пересматривать сделку нечем ни одним числом. Отбор вселенной это состояние
  // знает и называет сиротой (`universe-scan.js`), а автомат до 18.09 уходил в правило с пустым
  // кадром и получал от него `defer("short_history")` каждый каданс, то есть код «истории меньше
  // горизонта» у рынка, у которого истории 720 часов (находка 6.9 аудита механики).
  "held_missing",
...
//
// ДВА КОДА СТОРОЖА ЗАЛОГА ДРУГ С ДРУГОМ НЕ СОРЕВНУЮТСЯ, и знать это важно, чтобы не читать их
// порядок как правило. `marginGuard` возвращает ОДИН вердикт по обеим ногам, и в журнал едет ровно
// один его код, поэтому соседство `margin_unknown` и `margin_thin` здесь ничего не решает. Кто
// сильнее, когда верны оба факта (одна нога измеренно тонка, другая не посчитана), решается внутри
// самого сторожа: сильнее измеренная опасность, потому что ликвидация ноги невосстановима. До
// 18.09 сторож до этого вопроса не доходил вовсе, он выходил на первой непосчитанной ноге.
...
const REFUSAL_TEXT = Object.freeze({
  ...
  held_missing: "рынка, на котором мы стоим, в срезе нет вовсе",
...
export function armAuto(st, { nowMs, params = null, mode = "continuous" } = {}) {
  ...
  s.uptime = { ticks: 0, firstAt: null, lastAt: null, maxGapMs: 0, gaps: [], nominalSec: null };
  s.lastRefusals = [];
  // ВЗВОД ТРЕБУЕТ РЕШЕНИЯ НЕМЕДЛЕННО, А НЕ ЧЕРЕЗ КАДАНС (находка 6.4 аудита механики 18.09).
  // `PLAN-bot1-auto.md` раздел 7 дословно: «если true - немедленно триггерит decision-тик вместо
  // ожидания каданса, аналог `sellChainNextTryAt = 0` у бота 2 при включении». Метку решения взвод
  // не трогал вовсе, поэтому после каждого ручного включения бот мог простоять до 24 часов.
  // Обнуление метки это и есть тот самый триггер: `shouldDecideNow` на пустой метке отвечает «да».
  s.lastDecisionAt = null;
  // Снимок событий тоже снимается: он меряется ОТ ПРОШЛОГО решения, а прошлого решения у только
  // что взведённого автомата нет. Оставить чужой снимок значило бы мерить событие от чисел,
  // относящихся к другому периоду работы.
  s.lastDecisionCtx = null;
  return s;
}
...
export function autoTick({
  ...
  // ЧЕГО В ЗАМОРОЖЕННОМ НАБОРЕ НЕ БЫЛО. Поля, приехавшие из сегодняшних умолчаний, а не из взвода
  // (см. блок «ГРАНИЦА ЭТОГО ЗАКОНА» в шапке): без этого списка выкатка меняла бы поведение
  // открытой сделки молча. Пустой список у сделки, взведённой полным набором.
  const paramsFromDefaults = st.params
    ? Object.keys(defaultAutoParams()).filter((k) => !(k in st.params))
    : Object.keys(defaultAutoParams());
  const preset = faSizingPreset(params.presetId) || {};
  ...
  //
  // ЧТО ВОРОТА ВЫРАВНИВАЮТ, А ЧТО НЕТ (находка 6.11 аудита механики 18.09). Они режут кадр РОВНО до
  // окна и требуют покрытия базами не ниже `baseCoverageMin`, то есть выравнивают число СТРОК и
  // ограничивают снизу число часов с базой. Они НЕ выравнивают число ПОСЧИТАННЫХ часов: час без
  // базы доход обнуляет, а борроу и ногу Hyperliquid начисляет полностью, поэтому рынок с покрытием
  // 95% приходит на сравнение с брутто, занижённым на свои 5%, против соседа со 100%. Искажение
  // ограничено самими воротами (не больше `1 - baseCoverageMin`), измеримо по полям кривой
  // (`hoursApplied`, `noBaseSec`, `zeroBaseSec`, `badBaseSec`) и НЕ выправляется здесь: приведение
  // брутто к полному окну это правка правила, двигающая обе книги, и она требует решения владельца.
  const usable = [];
  ...
  let heldGate = null;
  // ВИДЕЛИ ЛИ МЫ УДЕРЖИВАЕМЫЙ РЫНОК В СРЕЗЕ ВООБЩЕ. Отдельный признак от `heldGate`: тот говорит,
  // с каким исходом рынок прошёл ворота, и у выпавшего из среза рынка исхода нет вовсе (ворота
  // ставятся только внутри цикла по рынкам). Без этого признака оба состояния были неразличимы.
  let heldSeen = false;
  let anyShort = false;
  ...
  for (const m of markets || []) {
    const rows = m?.rows || [];
    if (isHeld(m?.token ?? null)) heldSeen = true;
    if (rows.length < W) {
      anyShort = true;
  ...
  // РЫНОК, НА КОТОРОМ МЫ СТОИМ, ВЫПАЛ ИЗ СРЕЗА ЦЕЛИКОМ. Сильнее любых ворот: у ворот есть исход, а
  // здесь нет и рынка. Решать по такому состоянию нельзя, и молчать о нём тоже: до 18.09 тик уходил
  // в правило выхода с пустым кадром и получал `defer("short_history")` каждый каданс до
  // перезагрузки приложения, то есть сделка висела без пересмотра под верным на вид кодом.
  if (position && !heldSeen) add("held_missing", { where: "held", token: heldToken });
  else if (heldGate) add(heldGate, { where: "held", token: heldToken });
  ...
  const out = (kind, why, extra = null) => {
    ...
    return {
      ...
      window: null, evalMarkets: null, params, paramsFromDefaults, cfg, trigger: null, decisionEvents, ...(extra || {}),
    };
  };
  ...
  if (position) {
    ...
    // КАДАНС И СНИМОК СОБЫТИЙ ДВИГАЮТСЯ ТОЛЬКО ТОГДА, КОГДА РЕШЕНИЕ СОСТОЯЛОСЬ (находка 6.2 аудита
    // механики 18.09). До правки обе метки ставились СРАЗУ после правила, то есть и на исходе
    // `defer`, когда правило решать отказалось (`src_gmx_down`, `src_hl_down`, `short_history`,
    // `horizon_missing`, `window_missing`). Следствия было два, и второе дороже первого:
    //   * каданс тратился на отказ снабжения, то есть решение отодвигалось на сутки. Порядок цены
    //     даёт шапка `exit.js`: устаревание решения на 24 ч снимает 6.6% годового нетто, то есть
    //     час задержки стоит около $2 годового нетто;
    //   * снимок событий переписывался УЖЕ СДВИНУТЫМИ числами, и событие исчезало без решения: у
    //     `neg_streak` проверка `hours >= k && before < k` после обновления снимка даёт
    //     `before >= k` навсегда для этой полосы, а у `pot_drop` и `room_drop` отсчёт начинался от
    //     уже упавшего значения.
    // Заодно снимается асимметрия внутри самого автомата: блокирующий отказ каданс не тратил (он
    // решает выше метки), а отказ снабжения, дошедший до правила, тратил.
    if (ex.action !== "defer") {
      st.lastDecisionAt = now;
