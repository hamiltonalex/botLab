...
//
// НЕЙТРАЛЬНА К ЦЕНЕ В ДОЛЛАРАХ ПРИ ПЛЕЧЕ 1: залог в том же активе и есть противовес. Ход цены на x
// меняет залог на N*x и результат шорта на -N*x, в сумме ноль, поэтому леджер книжит по ней только
// фандинг и заимствование короткой стороны (`paper.js` `legModel`), а сторож залога, считающий
// залог в долларах, для неё пессимистичен (`fa/margin.js`). При плече L выше 1 остаётся короткая
// экспозиция N*(1 - 1/L); плечо владельца заморожено единицей.
//
// СХЕМЫ, МОНЕТЫ И ЦЕПИ ДЛЯ ПРАВИЛА ВХОДА РАВНОПРАВНЫ: оно считает все рынки вселенной одной
// экономикой (нетто за горизонт после круга издержек) и финансирует лучший; правило выхода
// сравнивает сделку со всеми альтернативами, включая другую схему. Схема в критерий не входит.
