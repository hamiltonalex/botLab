...
export function buildSnapshot(inst, gmxCanon, hlCanon) {
  ...
  const row = {
    ...
    // 0.38%, максимум 3.36%, при совпадении плательщика 20 из 20, тогда как часовой снимок
    // индексатора возрастом 37.6 минуты расходился уже на 14.71% и с разнобоем по сторонам. То
    // есть свежесть здесь важнее точности поля, а Subsquid для входа не годится.
    //
    // ЭТО ДОПУЩЕНИЕ, И У НЕГО ЕСТЬ СРОК ГОДНОСТИ. У GMX есть флаг `useOpenInterestInTokensForBalance`,
    // и от него зависит, чему равна база: интересу в долларах или интересу в токенах, пересчитанному
    // по цене. Флаг переключается: в записи репозитория ровно одно переключение, 2025-12-22T15 UTC,
    // одновременно на всех рынках, и с тех пор он равен true. Названная сверка 0.38% сделана ПОСЛЕ
    // переключения, то есть покрывает только текущий режим. Сама подмена поля при этом не остаётся
    // незамеченной: она ломает ОТНОШЕНИЕ сторон, а `resolveBase` сверяет тождество и отказывает
    // причиной `base_identity_broken`. Замер силы этой проверки: подстановка интереса в токенах
    // вместо базы даёт невязку 0.92% у BTC и 20.6% у ETH и отвергается в 4448 часах из 4448.
    //
    // ССЫЛКА НА ЭТУ ОХРАНУ СНОВА ВЕРНА, И ПОЧТИ ТРИ НЕДЕЛИ НЕ БЫЛА (находки 6.8 и 7.12 аудита
    // механики 18.09). Строка `baseIdentityOk` среза правил питалась ДРУГОЙ проверкой (тождество
    // `netRate == funding + borrow` внутри ответа GMX), то есть тождества баз живой снимок не
    // сверял нигде, а эта шапка ссылалась на охрану как на действующую и приводила числа замера,
    // снятого при строгом пороге 1e-6. С 18.09 срез считает тождество баз сам и тем же `resolveBase`
    // (`fa/slice.js`), порог строгий, и замер по копии живой записи mb12 (18 110 наблюдений)
    // показал невязку медиана 2.9e-10, максимум 1.3e-7, то есть ни одного отказа на здоровых
    // данных. Почасовая проверка в леджере при этом осталась своей и с ДРУГИМ допуском: у строки
    // кадра базы живые, а ставки индексаторские, и для неё допуск 5% (см. шапку `dilution.js`),
    // который подмену интереса в токенах у BTC (0.92%) и APT (1.68%) не ловит. Это два разных
    // места с двумя разными порогами, и путать их нельзя.
