...
//
// КОМИССИЯ ПЕРПА ЗДЕСЬ - ДОПУЩЕНИЕ ЗАМЕРА, А НЕ СТАВКА БИРЖИ. `SELLHEDGE_DEFAULTS.perpFee = 0` это
// условие, при котором сняты пятилетние книги охраны (test/baselines/books.sha256) и калибрована
// полоса 0.03; менять его значит осознанно сдвинуть обе книги схемы продавца. Живой тракт бота 2
// считает иначе: btcopt/hedge.js книжит каждое исполнение хеджа по ставке биржи из снимка перпа
// (deribit.js кладёт maker_commission / taker_commission меты инструмента, 2026-09-05: 0.00015 и
// 0.00035), а константы движка там лишь запасные. Поэтому бумага пессимистичнее замера ровно на
// комиссию оборота хеджа (прогон 17.08-05.09: $32.83 на обороте $218 870), и это ожидаемое
// расхождение, а не разные правила: чувствительность схемы к этой статье измерена (стрэнгл 336-672
// ×4.09 при 0 б.п., ×3.18 при 2.5 б.п.), и стенды принимают ставку параметром `--perp-fee`.
...
export const SELLHEDGE_DEFAULTS = Object.freeze({
  ...
  perpFee: 0, // доля от оборота перпа: ДОПУЩЕНИЕ ЗАМЕРА, книги охраны сняты при нуле; живой тракт берёт ставку биржи из снимка перпа (см. шапку)
  ...
  stop: null,
  // Наклон размера от разрыва IV-RV. null = наклона нет, и это боевой дефолт: пятилетние книги
  // сняты схемой, у которой размер решает только стресс-правило. Поле существует ради ЗАМЕРА по
  // предрегистрации 2026-09-22: ответом на разрыв IV-RV там меряется ИЗМЕНЕНИЕ РАЗМЕРА, а не
  // запрет входа, потому что запрет уже замерен дважды и оба раза проиграл (см. sizeTiltMult).
  // Форма: { loMult, hiMult, spanPts }. Пока значение null, ни одна ветка не исполняется.
  sizeTilt: null,
});

// ── ИМЕНОВАННЫЕ ПРЕСЕТЫ СХЕМЫ. Реестр по образцу `SCAN_PRESETS` (presets.js), но для правил
// ПРОДАВЦА: тот реестр обслуживает покупательский чеклист и правил этой схемы не знает.
//
// ЗАЧЕМ ИМЕНА, ЕСЛИ ПЕРЕКРЫТИЕ И ТАК РАБОТАЛО. Конфигурация схемы выражалась безымянным
// перекрытием `sellCfg` в месте вооружения, и у неё не было ни имени, ни версии. Следствий два, и
// оба про доверие к числам. Первое: про открытую сделку нельзя было сказать, каким набором правил
// она открыта - паспорт нёс отдельные поля, но не то, СКОЛЬКО их было и все ли пришли из одного
// замера. Второе: замеренную конфигурацию нельзя было отличить от собранной на ходу, потому что
// обе выглядят одинаково - объектом с полями. Имя и пометка `calibrated` отвечают ровно на это.
//
// ПРЕСЕТ ЭТО ЧИСТЫЕ ДАННЫЕ И НИЧЕГО БОЛЬШЕ. Он накладывается тем же механизмом `sellCfg`, каким
// накладывалось перекрытие; ветки исполнения от пресета не зависят и новых путей не появилось.
// Поэтому вооружение пресетом и вооружение теми же полями россыпью дают ПОБИТОВО один результат,
// и это проверяется тестом, а не обещается.
//
// ЧЕГО В ПРЕСЕТЕ НЕТ НАМЕРЕННО:
//   `spreadScale` - это не правило схемы, а поправка на МОДЕЛЬНЫЕ котировки восстановленной записи
//                   (1.10 против 1 на живых). Свойство снабжения, и живой тракт ставит своё;
//   `stop`        - уровень досрочного выхода СЧИТАЕТСЯ кодом от потолка размера (`liveSellStop`),
//                   потому что оба числа описывают один аппетит к риску. Записанный в пресет, он
//                   разъехался бы с потолком при первой правке одного из них;
//   `deployPct`   - потолок развёртывания остаётся дефолтом схемы: боевой размер считает
//                   стресс-правило, а deployPct нужен ещё и сканеру, который меряет ВОЗМОЖНОСТЬ.
const sellPreset = (p) => ({ ...p, cfg: Object.freeze({ ...p.cfg, presetId: p.id }) });

// Боевой. Числа не выбраны на глаз и не собраны заново: это ровно те окно, дельта и полоса, на
// которых сняты пятилетние книги (см. разбор у SELLHEDGE_DEFAULTS), плюс автономное правило
// размера X=45 / cap=0.8, которым цепочка вооружается с 2026-08-26.
const SELL_CALL_336_672_V1 = sellPreset({
  id: "sell-call-336-672-v1",
  label: "Продавец колла 336-672 ч",
  calibrated: true, // подтверждён пятилетними книгами и сеткой размера
  cfg: {
    expiryMinH: 336,
    expiryMaxH: 672,
    deltaTarget: 0.45,
...
// Короткий. Отличается ОДНИМ окном срока (168-336 ч, то есть 7-14 суток) и существует ради замера:
// перебор срока по пяти годам давал на этом окне ×1.99 при просадке 18.6% против боевых ×2.03 при
// 13.5%, то есть кандидат правдоподобный и НЕ проверенный подтверждениями - книг им не снимали,
// сетку размера на нём не гоняли. Поэтому `calibrated: false`, и пометка обязана быть видна и в
// интерфейсе, и в паспорте сделки: конфигурация, которую никто не мерил, не должна выглядеть как
// измеренная только потому, что у неё появилось имя.
const SELL_CALL_168_336_V1 = sellPreset({
  id: "sell-call-168-336-v1",
  label: "Продавец колла 168-336 ч",
  calibrated: false,
  cfg: { ...SELL_CALL_336_672_V1.cfg, expiryMinH: 168, expiryMaxH: 336 },
});

const deepFreezeSell = (o) => {
  for (const v of Object.values(o)) if (v && typeof v === "object") deepFreezeSell(v);
  return Object.freeze(o);
};

export const SELL_PRESETS = deepFreezeSell({
  [SELL_CALL_336_672_V1.id]: SELL_CALL_336_672_V1,
  [SELL_CALL_168_336_V1.id]: SELL_CALL_168_336_V1,
});

// Пресет по умолчанию: им вооружается цепочка и его окном живёт режим продажи сканера.
export const SELL_PRESET_DEFAULT_ID = SELL_CALL_336_672_V1.id;

export const isKnownSellPreset = (id) => typeof id === "string" && Object.hasOwn(SELL_PRESETS, id);

// resolveSellCfg(sellCfg) - ЕДИНСТВЕННОЕ место, где дефолты схемы, пресет и перекрытие вызывающего
// сходятся в рабочую конфигурацию. До этой функции одно и то же слияние было написано семь раз, и
// добавить в него разбор имени пресета значило бы семь одинаковых правок с одной однажды забытой -
// ровно тот класс дефекта, ради которого правила схемы вообще переехали в этот модуль.
//
// ПОРЯДОК СЛИЯНИЯ ЗНАЧИМ: дефолты, потом пресет, потом ЯВНОЕ перекрытие вызывающего. Явное
// побеждает пресет намеренно: офлайн-замер меняет одно поле поверх именованной конфигурации и
// обязан получить именно его, а не молча вернуться к пресету.
//
// НЕИЗВЕСТНОЕ ИМЯ НЕ РОНЯЕТ ТРАКТ, а разрешается в боевой пресет (идиома `resolveScanPreset`):
// сюда заходят и живой тик, и прогон записи, и падать посреди них из-за опечатки в имени нельзя.
// Место, где опечатка обязана быть ГРОМКОЙ, одно - вооружение оператором, и там стоит
// `isKnownSellPreset` с называемой причиной отказа.
//
// БЕЗ АРГУМЕНТА ФУНКЦИЯ ТОЖДЕСТВЕННА ПРЕЖНЕМУ СЛИЯНИЮ: `{ ...SELLHEDGE_DEFAULTS }` и ни поля
// больше. Офлайн-эталон и прогон записи имени пресета не передают, поэтому пятилетние книги этой
// правкой не двигаются - проверяется тестом и сверкой сумм книг.
export function resolveSellCfg(sellCfg = null) {
  const over = sellCfg ?? {};
  if (over.presetId == null) return { ...SELLHEDGE_DEFAULTS, ...over };
  const preset = SELL_PRESETS[over.presetId] ?? SELL_PRESETS[SELL_PRESET_DEFAULT_ID];
  return { ...SELLHEDGE_DEFAULTS, ...preset.cfg, ...over, presetId: preset.id };
}
...
export function shouldStopOut({
  ...
  switch (metric) {
    ...
    // счёта пережить СЛЕДУЮЩИЙ ход, а не на уже понесённый минус. Счёт ноль и ниже означает, что
    // биржа закрыла бы позицию сама, поэтому это безусловное срабатывание, а не деление на минус.
    // Неизвестный капитал (не число) это молчание, а известный неположительный уже срабатывание:
    // прежде оба случая отсекала одна проверка на положительность, и живой бот с капиталом ниже
    // нуля получал ответ «стопа нет».
    case "mmu": {
      if (!posNum(mmUsd) || !posNum(qty) || !fin(equityUsd) || !fin(mtm1)) return false;
...
// ── ПРАВИЛО ВЫХОДА В БОЮ: РЕШАЕТ КОД, А НЕ ТУМБЛЕР.
//
// Тот же принцип, что у автономного размера позиции: величину, которую можно вывести из замера,
// оператор не выбирает. Экрана у правила выхода нет и не заводится - живой тракт вооружает его сам
// при каждом открытии, а перекрытие существует только для замеров и прогонов.
//
// ЧТО ИМЕННО ЗАМЕРЕНО (предрегистрация 2026-08-28, 100 клеток на 84 сделках пятилетки).
// Выигрывает ОДНА метрика из пяти - утилизация поддерживающей маржи, и выигрывает она хвост, а не
// доходность: 99-й процентиль загрузки падает на 15-18 п.п. ценой 1.4% роста, правило срабатывает
// 4 раза за пять лет. Стоп по УБЫТКУ (доля премии, залога, счёта) проверен и отвергнут: 21 клетка
// из 28 не срабатывает вовсе, потому что дельта-хедж держит убыток внутри одной проданной премии,
// а все семь сработавших ухудшают итог. Стоп по расстоянию до страйка отвергнут как артефакт
// калибровки: при одинаковом размере позиции он теряет деньги.
//
// ПОЧЕМУ УРОВЕНЬ СЧИТАЕТСЯ ОТ ПОТОЛКА РАЗМЕРА, А НЕ ЗАПИСАН ЧИСЛОМ. Правило размера уже держит
// стресс-маржу не выше `stressCapFrac` счёта, то есть ОБА числа описывают один и тот же аппетит к
// риску. Записанные порознь, они разъедутся при первой же правке одного из них, и бот начнёт
// выходить позже, чем сам себе разрешил занимать. Доля 0.75 не выведена из теории: она взята из
// замера (при потолке 0.8 лучшая по «срезанный хвост на единицу потерянного роста» клетка это
// 0.60), и записана долей ровно затем, чтобы связь с потолком не потерялась.
const STOP_LEVEL_OF_CAP = 0.75;

export function liveSellStop(cfg = SELLHEDGE_DEFAULTS) {
  if (cfg?.stop) return cfg.stop; // явное перекрытие: замер, прогон записи, ручной эксперимент
  const cap = posNum(cfg?.stressCapFrac) ? cfg.stressCapFrac : SELLHEDGE_DEFAULTS.stressCapFrac;
  return Object.freeze({
    metric: "mmu",
    level: Math.round(STOP_LEVEL_OF_CAP * cap * 1000) / 1000,
    action: "exit",
    // Два ШАГА подряд за порогом, чтобы одиночный выброс котировки не закрывал сделку. Шаг это вызов
    // затвора, а не час. В замере 2026-08-28 шаг был часовым, и два шага значили два часа. Живой бот
    // зовёт затвор на каждом тике раз в 15 секунд, то есть закрывает сделку через 15 секунд после
    // первого пробоя. Прогон тех же пяти лет по пути индекса с шагом 15 секунд (2026-09-24) дал у
    // стрэнгла те же 4 выхода из 84 сделок, что и два часовых наблюдения, только раньше внутри
    // сделки: рост 0.972 от базы против 0.980, пик загрузки маржи 65.1% против 67.9%.
    hyst: "band",
    fill: "same",
  });
}
...
export function sellhedgeEngineCfg(cfg = SELLHEDGE_DEFAULTS) {
  return {
    ...
    // ИМЯ КОНФИГУРАЦИИ ЕДЕТ В ЗАМОРОЖЕННЫЙ ПАСПОРТ ВМЕСТЕ С ЕЁ ЧИСЛАМИ. Иначе по сделке видно, с
    // какими окном, дельтой и полосой она открыта, но не видно, БЫЛ ЛИ ЭТО ЗАМЕРЕННЫЙ НАБОР или
    // числа, сошедшиеся случайно. Пометка замера копируется рядом с именем, а не смотрится потом
    // в реестре: паспорт обязан пережить уточнение и переименование пресета, то есть отвечать на
    // вопрос «что было правдой в момент открытия», а не «что правда сейчас».
    presetId: cfg.presetId ?? null,
    presetCalibrated: cfg.presetId != null ? SELL_PRESETS[cfg.presetId]?.calibrated ?? null : null,
  };
}
...
// ── МНОЖИТЕЛЬ РАЗМЕРА ОТ РАЗРЫВА IV-RV ────────────────────────────────────────────────────────
//
// ЗАЧЕМ. Результат продавца решает одна величина: проданная подразумеваемая волатильность минус
// та, которая реализуется за время удержания. Живая сделка 1 продала IV 33.08% при реализованной
// 45.9% и потеряла $789.84; сделка 2 продала около 36% при реализованной 35.4% и стоит около нуля.
// В решении о входе эта величина сегодня не участвует никак: `maybeOpenNextSell` открывает сделку,
// как только в окне срока нашлась нога с нужной дельтой.
//
// ПОЧЕМУ МНОЖИТЕЛЬ, А НЕ ЗАПРЕТ ВХОДА. Ответ «не открывать, пока разрыв мал» замерен дважды и оба
// раза проиграл: 2026-08-24 на схеме одной проданной ноги (лучшая из девяти клеток дала рост
// залога в 33.38 раза против базовых 45.64) и 2026-09-22 на стрэнгле при боевом размере (все шесть
// клеток сетки порогов ниже базы ×4.12). Механизм проигрыша один и он виден в отчёте: запрет не
// выбирает ногу, её выбирает правило срока и дельты, поэтому он умеет только ОТЛОЖИТЬ вход и
// платит за это простоем, доходящим до 21.9% записи. У множителя простоя нет по построению:
// капитал остаётся в деле, меняется только число контрактов.
//
// ФОРМА: множитель = 1 + (IV − RV7d) / spanPts, зажатый между loMult и hiMult. Якорь взят в нуле
// разрыва, и там множитель равен ровно единице. Ноль разрыва означает, что проданная волатильность
// равна недавно реализованной, то есть сделка открыта по справедливой цене и получает номинальный
// размер; это единственная точка на оси, не выбранная из данных. spanPts это ширина разрыва в
// пунктах волатильности, за которую множитель проходит единицу.
//
// МНОЖИТЕЛЬ НИКОГДА НЕ НОЛЬ, и это определение, а не осторожность: ноль был бы тем же запретом
// входа другими словами, то есть тем, что уже замерено и проиграно.
//
// НАКЛАДЫВАЕТСЯ ПОВЕРХ СТРЕСС-ПРАВИЛА РАЗМЕРА (lotsByStressMargin в btcopt/margin.js), а не вместо
// него: стресс-правило стережёт хвост, а множитель распоряжается тем, что стресс-правило
// разрешило. Обратный порядок означал бы, что жирная премия разрешает размер, который не переживёт
// ход спота на 45%.
//
// ДЕФОЛТ null, И ПРИ НЁМ НИ ОДНА ВЕТКА НЕ ИСПОЛНЯЕТСЯ. Пятилетние книги охраны сняты схемой без
// этого правила, и их арифметика обязана остаться прежней ДО БИТА. Тот же приём у `stop` выше.

// parseSizeTiltSpec("0.7:1.2:10") → { tilt: { loMult, hiMult, spanPts }, error: null }.
// Три числа через двоеточие: нижняя граница, верхняя граница, ширина полосы разрыва в пунктах
// волатильности. Ошибка НАЗЫВАЕТ виновника: спецификация приходит из командной строки, и «не
// разобралось» без адреса означает молча посчитанный не тот прогон.
export function parseSizeTiltSpec(spec) {
  const src = typeof spec === "string" ? spec.trim() : "";
  if (!src) return { tilt: null, error: "пустая спецификация (пример: 0.7:1.2:10)" };
  const parts = src.split(":").map((x) => x.trim());
  if (parts.length !== 3) {
    return { tilt: null, error: `ожидается «низ:верх:ширина», получено «${src}» (${parts.length} част(и) вместо трёх)` };
  }
  const [loMult, hiMult, spanPts] = parts.map(Number);
  if (!fin(loMult) || !(loMult > 0)) {
    return { tilt: null, error: `низ «${parts[0]}» должен быть положительным числом: множитель, равный нулю, это запрет входа, а не размер` };
  }
  if (!fin(hiMult) || !(hiMult >= loMult)) {
    return { tilt: null, error: `верх «${parts[1]}» должен быть числом не меньше низа ${loMult}` };
  }
  if (!fin(spanPts) || !(spanPts > 0)) {
    return { tilt: null, error: `ширина «${parts[2]}» должна быть положительным числом пунктов волатильности` };
  }
  return { tilt: { loMult, hiMult, spanPts }, error: null };
}

// Подпись правила для шапки отчёта и для ключа мемоизации цепочки.
export function formatSizeTilt(tilt) {
  return tilt
    ? `размер ×${tilt.loMult}..×${tilt.hiMult} от разрыва IV−RV, единица на нуле, полоса ${tilt.spanPts} п.в.`
    : "размер без наклона";
}

// sizeTiltMult({ tilt, ivPct, rv7dPct }) → { mult, gapPts, hasData }.
//
// БЕЗ ДАННЫХ МНОЖИТЕЛЬ РАВЕН ЕДИНИЦЕ, А НЕ НУЛЮ И НЕ ВЕРХНЕЙ ГРАНИЦЕ. Час без RV7d в записи это
// нехватка снабжения, а не сигнал рынка. Ноль превратил бы правило в запрет входа на этих часах,
// то есть подменил бы измеряемое; верхняя граница раздала бы размер там, где о разрыве ничего не
// известно. Единица означает «правило молчит, размер решает стресс-правило», и `hasData` даёт
// вызывающему сосчитать, как часто это случилось: молчание, которого никто не считает,
// неотличимо от работающего правила.
export function sizeTiltMult({ tilt, ivPct, rv7dPct } = {}) {
  if (!tilt) return { mult: 1, gapPts: null, hasData: false };
  if (!fin(ivPct) || !fin(rv7dPct)) return { mult: 1, gapPts: null, hasData: false };
  const gapPts = ivPct - rv7dPct;
  const raw = 1 + gapPts / tilt.spanPts;
  return { mult: Math.min(tilt.hiMult, Math.max(tilt.loMult, raw)), gapPts, hasData: true };
}
