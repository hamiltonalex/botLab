...
test("файл эталонов репозитория разбирается и описывает четыре книги: две продавца и две бота 1", () => {
  ...
  assert.deepEqual([...entries.keys()].sort(), ["base-eng.tsv", "base-fa-dil.tsv", "base-fa.tsv", "base-ref.tsv"]);
  ...
  // Книги бота 1 снимает ОДИН скрипт в двух режимах, и одинаковая сумма означала бы, что режим
  // разбавления не исполнился вовсе: без баз брутто $1189.17, с базами $-21.98.
  assert.equal(entries.get("base-fa-dil.tsv").length, 64);
  assert.notEqual(entries.get("base-fa-dil.tsv"), entries.get("base-fa.tsv"));
});
