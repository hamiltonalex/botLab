...
test("бесконечных анимаций на постоянных состояниях в разметке нет", () => {
  ...
  // (258 МБ за 11 часов) и постоянная нагрузка ради мигающей точки. Мигание обязано быть
  // одноразовым, на приход снимка. Единственное допустимое `infinite` это точка проверки
  // обновления: состояние `checking` длится секунды и само собой заканчивается.
  const rules = [...HTML.matchAll(/([^{}\n]+)\{[^{}]*animation:[^;}]*\binfinite\b[^{}]*\}/g)].map((m) => m[1].trim());
  assert.deepEqual(rules, [".verpill.checking .vdot"], `бесконечные анимации: ${rules.join(" | ")}`);
  // Одноразовое мигание стоит на всех трёх пилюлях и идёт через тот же приём, что остальные blip'ы.
  assert.match(HTML, /\.live\.fresh\.tick \.led\{ animation:pulse [^;}]*\b1; \}/, "у пилюли LIVE одноразовое мигание");
  for (const call of ["ledBlink($('liveBtn'), f.gmxAtIso)", "ledBlink(btn, f && f.atIso)", "ledBlink(btn, fresh.atIso)"]) {
    assert.ok(HTML.includes(call), `мигание на новом снимке: ${call}`);
  }
  assert.match(HTML, /function ledBlink\([^)]*\)\{[^}]*smoothOk\(\)/, "мигание уважает prefers-reduced-motion");
});
