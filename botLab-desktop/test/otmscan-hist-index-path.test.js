...
// НАЗВАННУЮ ошибку, а не молча посчитанный не тот прогон; (3) обрезанный файл ловится по длине,
// а не читается за концом буфера; (4) поиск шага находит СТРОГО следующий и не ломается на
...
test("чужой файл НАЗЫВАЕТ себя ошибкой, а не читается как таблица", () => {
  // Таблицу подают путём из командной строки. Молчаливый отказ означал бы прогон по мусору.
...
test("обрезанный файл ловится по длине, а не читается за концом буфера", () => {
  // Заголовок обещает три шага, байт хватает на один: это оборванная закачка, и она обязана
  // назваться, потому что иначе прогон пошёл бы по мусорным ценам из соседней памяти.
