...
test("оборванный хвост после падения: читатель выживает, потеря названа числом", () => {
  ...
  try {
    ...
    // ДОПИСЫВАНИЕ ПОСЛЕ ОБРЫВА ТЕРЯЕТ РОВНО ОГРЫЗОК, И БОЛЬШЕ НИЧЕГО. Раньше новая строка
    // приклеивалась к огрызку и пропадала вместе с ним: падение уносило две записи, причём вторая
    // была целой и написанной уже после восстановления.
    appendScanRecords(dir, P, D1, [{ n: "после-обрыва" }]);
    const after = readScanRecords(dir, P, [D1]);
    assert.deepEqual(after.rows.map((r) => r.n), ["целая-1", "целая-2", "после-обрыва"],
      "строка, записанная после падения, читается");
    assert.equal(after.broken, 1, "битой осталась одна строка: тот самый огрызок");
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
